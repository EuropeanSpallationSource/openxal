 ...
 * noCorrelationCaught) at a specified period. Only the most recent best
 * correlation within a given time interval will be posted unless there are
 * other full count correlations in which case they will be posted immediately.
 * Other correlations within that same time period will be dropped. This
 * broadcaster may be set to repeat or to fire only once.
 *
 * @author tap
 */
class TimedBroadcaster<T> extends AbstractBroadcaster<T> {
    ...
    /**
     * Handle the BinListener event by comparing the number of correlated
     * records and saving it as the best correlation if it has more than the
     * previous best. It never posts events, but the best correlation can be
