...
/**
 * <p>
 * Implementation of a linear, fixed-length data buffer. The current version
 * follows a list implementation and, thus, the collection is naturally ordered
 * according to their relative point of insertion.
 * </p>
 * <p>
 ...
 * <code>{@link CircularBuffer}</code> are the iterators provided. The
 * <code>CircularBuffer</code> class uses a Last In First Out (LIFO) iteration
...
public class LinearBuffer<T> implements IBoundedList<T> {
    ...
    /**
     * Adds an object to the buffer, pushing off an old object if the buffer is
     * full.
     *
    ...
    /**
     * Return the object at the specified position in this buffer. The buffer
     * uses a last in first out (LIFO) policy.
     *
     ...
     */
    /**
     * Return an iterator object for this buffer which traverses the buffer
     * objects in Last In First Out order.
     *
     ...
     */
    /**
     * Return an iterator object for this buffer which starts with the object at
     * the given index and ends with the last object in the buffer.
     *
