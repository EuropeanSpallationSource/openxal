 ...
 * formatting is not satisfied under Java 8. Use ScientificNumberFormat instead.
 * This class is around temporarily for backward compatibility but will be
 * removed in the near future. FortranNumberFormat is the subclass of the
 * DecimalFormat class. It formats the double numbers and integers according to
 * GN.F format specification in the FORTRAN programming language. Usually N = F
 * + 7, because an exponential representation needs additional symbols for signs
 * of the value and the power of ten, E symbol, a decimal point, and the value
 * of power. For instance, -1.23E-101. If user will choose N < F + 7 the
 * formatting still will work, but the length of the resulting string may be
 * more that N. The property fixedLength can be used to fill out the resulting
 * string with leading spaces. This property can be set with <code> setFixedLength
 * </code> method or by using a proper constructor
 *
 * @author shishlo
 * @version 1.0
 */
