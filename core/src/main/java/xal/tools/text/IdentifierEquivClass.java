...
/**
 * <p>
 * Represents an equivalence class of element identifiers. An element ID is a
 * member of the equivalence class if it is a proper substring of the current
 * class representative or the current class representative is a proper
 * substring of the given element ID. The class representative identifier is
 * continually truncated whenever a member element ID is encounter that is
 * shorter, that is, the encountered ID becomes the representative ID.
 * </p>
 * <p>
 * The class of identifiers is intended to be associated with one hardware
 * element.
 * </p>
 * <p>
 ...
 * counterpart. Thus, we equate two <code>ElementIdClass</code> objects if they
...
public class IdentifierEquivClass implements Comparable<IdentifierEquivClass> {
     ...
     */
    /**
     * Checks whether or not the given ID is an member of this equivalence class
     * of element IDs.
     *
     * @param strElemId element identifier under comparison
     *
     * @return              <code>true</code> if argument is equivalent to the representative
     * element of this equivalence class, <code>false</code> otherwise
     *
     * @author Christopher K. Allen
     * @since Jun 10, 2013
     */
     ...
     *
     * @return returns <code>true</code> if the argument and this object
     * represent the same equivalence class, or <code>false</code> otherwise
     *
     * @author Christopher K. Allen
     * @since Jun 10, 2013
     */
     ...
     */
    /**
     * Implements a comparison of equivalences classes for the element IDs. We
     * first check whether or not the argument is equivalent to this object, if
     * so a value of 0 is returned. If not, then the representative strings of
     * the equivalence class are compared lexicographically, the result of that
     * comparison is returned. Note that is value will not be 0 since the
     * objects under comparison are not in the same equivalence class of element
     * IDs.
     *
     * @param idcCmp equivalence class object to compare w.r.t. the element ID
     * class
     *
     * @return a value 0 if the argument is the same equivalence class, or the
     * lexicographical comparison result otherwise
     *
...
//        System.out.println("lexicographic compareTo(" + this.strClassId + ", " + idcCmp.strClassId + ") = " + iResult);
     ...
     */
    /**
     * Checks whether or not the given object is equivalent to this object and
     * also sets the base ID string if necessary. Two
     * <code>ElementIdentifiers</code> are equivalent if one is a substring of
     * the other. The base ID string then becomes the smallest common string
     * between them.
     *
     * @param obj obj to be compared (must be of type
     * <code>ElementIdentifier</code>
     *
     * @return returns <code>true</code> if the given element identifier object
     * is equivalent to this (see above), <code>false</code> otherwise.
     *
...
//        System.out.println("equals(x) with this.strClassId=" + this.strClassId + " and x.strClassId=" + idcCmp.strClassId);
    ...
    /**
     * <p>
     * This is essentially a utility method for the other methods of the class,
     * as such I have removed the visibility.
     * </p>
     * <p>
     * Makes a comparison of the equivalence class representative string ID with
     ...
     * is returned (described below). The empty identifier <b>""</b> is handled
     ...
     * <tt>s</tt> the given argument string and by <tt>r</tt> the representative
     * string of this equivalence class (so that the equivalence class is
     * represented as [<tt>r</tt>]). Then the follow results are returned:
     * <br>
     * <br>
     ...
     * <tt>s</tt>[0] == <tt>r</tt>[0] must hold in every case since neither is
     * the empty string.
     * </p>
     *
     ...
     *
     * @return if the two strings are equivalent, then the result of the
     * comparison as described above, otherwise an exception is thrown.
     *
     * @throws NoSuchElementException the given string ID is not equivalent to
     * this object
     *
     * @author Christopher K. Allen
     * @since Jun 10, 2013
