...
public class Particle implements Serializable {
    ...
    public R3 getPosition() {
        return this.getPhase().getPosition();
    }
    ...
    public R3 getMomentum() {
        return this.getPhase().getMomentum();
    }
    ...
    /**
     * Computes electric potential assuming the particle was an uniform sphere
     * of charge with radius R. Giving the particle a finite size removes the
     * singularity in the potential at the particle position.
     *
     * This function is also useful in averaging the potential over grids. By
     * giving the radius the same order as the grid dimensions, the particle
     * appears to smear over the grid. That is, the radius is a numerical tuning
     ...
     * charge with radius R. Giving the particle a finite size removes the
     * singularity in the field at the particle position.
     *
     * This function is also useful in averaging the field over grids. By giving
     * the radius the same order as the grid dimensions, the particle appears to
     * smear over the grid. That is, the radius is a numerical tuning parameter
     * for averaging.
     *
     ...
     * magnetic flux vector B. Giving the particle a finite size removes the
     * singularity in the field at the particle position. Note that very large
     * fields exist sufficiently close to the particle.
     *
     * To avoid numerical singularities the particle is assumed to have a finite
     * radius equal to the "classical proton radius", which is ~1e-18. Thus, the
     * field of a uniform sphere is substituted for field points closer than
     * this radius.
     *
     * Note that if the momentum coordinates of the particle's phase space
     * vector are (x',y',z') then the returned field must be multiplied by
     * beta-c, since the mechanical momentum coordinates are assumed in the
     * field calculations.
     *
     * @param ptFld field point to evaluate the electric field
     *
