 ...
 * <code>SimResultsAdaptor</code> to the specific use of the calculation engine
 * <code>{@link CalculationsOnRings}</code> for simulation data of type
 * <code>TransferMapTrajectory</code>, and use of calculation engine
 * <code>{@link CalculationsOnBeams}</code> for simulation data of type
 * <code>EnvelopeTrajectory</code>. Thus, probes states of either type
 * <code>TransferMapState</code> or <code>EnvelopeProbeState</code> may be
 ...
 * <h3>NOTE:</h3>
 * <p>
 * - Calculations for the <code>ParticleProbeTrajectory</code> have been added.
 * The calculation engine is <code>CalculationsOnParticles</code>. Note that
 * only the methods of interface <code>ISimLocResults</code> will be recognized.
 * Methods of interface <code>ISimEnvResults</code> will results in an
 * exception.
 * </p>
 *
 * @author Christopher K. Allen
...
public class SimpleSimResultsAdaptor extends SimResultsAdaptor {
 ...
 /*
     ...
     */
    /**
     * Constructor for <code>SimpleSimResultsAdaptor</code>. We create an
     * internal machine calculation engine based upon the type of the given
     * simulation trajectory.
     *
     * @param trajectory simulation data that is going to be processed
     *
     * @throws IllegalArgumentException the simulation data is of an unknown
     * type
     *
     * @author Christopher K. Allen
     * @since Nov 7, 2013
