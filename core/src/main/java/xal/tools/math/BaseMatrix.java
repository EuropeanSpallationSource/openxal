 ...
 * the interacting objects are all of type <code>M</code>, or vectors of the
 * singular type <code>V</code>. The template parameter <code>M</code> is the
 * type of the child class. This mechanism allows
 * <code>BaseMatrix&lt;M extends BaseMatrix&lt;M&gt;&gt;</code> to recognize the
 * type of it derived classes in order to create and process new objects as
 * necessary.
 * </p>
 * <p>
 * The objective of this base class is to hide the internal implementation of
 * matrix operations from the child classes and all developers using the matrix
 * packages.
 * </p>
 * <p>
...
public abstract class BaseMatrix<M extends BaseMatrix<M>> implements IArchive {
     ...
     */
    /**
     * The default character width of matrices when displayed using
     * {@link #toStringMatrix()}
     */
     ...
     *
     * @exception ArrayIndexOutOfBoundsException an index was equal to or larger
    ...
    /**
     * Set the element specified by the given position indices to the given new
     * value.
     *
     * @param iRow matrix row location
     * @param iCol matrix column index
     *
     * @param dblVal matrix element at given row and column will be set to this
     * value
     */
    ...
    /**
     * Set a block sub-matrix within the current matrix. If the given
     * two-dimensional array is larger than block described by the indices it is
     * truncated. If the given indices describe a matrix larger than the given
     * two-dimensional array then an exception is thrown.
     *
    ...
    /**
     * Parsing assignment - set the matrix according to a token string of
     * element values.
     *
    ...
    /**
     * <p>
     * Returns the matrix element at the position indicated by the given row and
     * column index sources.
     * </p>
     * <h3>NOTES</h3>
     * <p>
     ...
     * The array dimensions are given by the size of this matrix.
     *
    ...
    /**
     * <p>
     * Tests whether the given matrix is approximately equal to this matrix. The
     * idea is that we ignore any numerical noise when comparing if the two
     * matrices are equal.
     * </p>
     * <p>
     * This is a convenience class for the method
     * <code>{@link #isApproxEqual(BaseMatrix,int)}</code> where the number of
     * ULPs is set to <code>ULPS_BRACKET</code>.
     * </p>
     * <p>
     ...
     * </p>
     *
     * @return  <code>true</code> if the given matrix is equal to this one with
     * the given number of significant digits, <code>false</code> otherwise.
     *
     ...
     * Last Place in the machine representation. The larger this number the more
     * least significant digits we ignore.
     * </p>
     * <p>
     * The matrices are compared element by element using
    ...
    /**
     * Create a deep copy of the this matrix object. The returned object is
     * completely decoupled from the original.
     *
    ...
    /**
     * Assign this matrix to be the zero matrix, specifically the matrix
     * containing all 0's.
     *
     * @author Christopher K. Allen
     * @since Oct 3, 2013
    ...
    /**
     * Assign this matrix to be the identity matrix. The identity matrix is a
     ...
     * Note that this method does a singular value decomposition just to get the
     * number (done in the (wasteful) EJML internal implementation). Thus, this
     * computation is not cheap if the matrix is large.
     *
    ...
    /**
     * Inverse of the matrix if the matrix is square, if the matrix is not
     * square this is a pseudo-inverse.
     *
     ...
     *
     * @return the value of this matrix minus the value of the given matrix, or
     ...
     *
     * @param matRight multiplicand - right operand of matrix multiplication
     * operator
     *
     * @return new matrix which is the matrix product of this matrix and the
     * argument, or <code>null</code> if an error occurred
     */
    ...
    /**
     * <p>
     * Return the maximum absolute value of all matrix elements. This can be
     ...
     * is the singular value operator.
     * </p>
     *
     * @return the maximum singular value of this matrix
     */
    public double norm2() {
        return NormOps.normP2(this.matImpl);
    }
     ...
     * <h3>NOTES</h3>
     * <p>
     * &middot; Since the Schatten norms are sub-multiplicative, the Frobenius
     * norm is sub-multiplicative.
     * <br/>
     * <br/>
     ...
     */
    /**
     * Save the value of this matrix to a data sink represented by the
     ...
     */
    /**
     * Checks absolute equivalency. That is, checks whether or not the argument
     * is this object.
     *
     ...
     * matrix entries are formatted. Thus, the string should resemble the usual
     * matrix format when printed out.
     *
     * @return multiple line formatted string containing matrix elements in
     * matrix format
     *
     * @author Christopher K. Allen
     * @since Feb 8, 2013
     ...
     * Returns a string representation of this matrix. The string contains
     * multiple lines, one for each row of the matrix. Within each line the
     * matrix entries are formatted according to the given number format. The
     * default column width is used. The string should resemble the usual matrix
     * format when printed out.
     *
     * @param fmt     <code>NumberFormat</code> object containing output format for
     * matrix entries
     *
     * @return multiple line formatted string containing matrix elements in
     * matrix format
     ...
     * multiple lines, one for each row of the matrix. Within each line the
     * matrix entries are formatted according to the given number format. The
     * string should resemble the usual matrix format when printed out.
     *
     * @param fmt         <code>NumberFormat</code> object containing output format for
     * matrix entries
     * @param intColWd number of characters used for each column (padding is
     * with spaces)
     *
     * @return multiple line formatted string containing matrix elements in
     * matrix format
    ...
    /**
     * Sets the internal matrix value to that given in the argument. This is a
     * deep copy operation. Note that the complete matrix is copy, thus the
     * dimensions and other parameters are assigned as well.
     *
     ...
     * </p>
     * <p>
     * NOTE: &middot; This method was made abstract by Ivo List. Rather than use
     * reflection to instantiate new objects, this function is now delegated to
     * the concrete classes. This architecture is more robust and allows the
     * compiler to do more error checking.
     * </p>
     *
     ...
     *
     * @throws UnsupportedOperationException child class has not defined a
    ...
    /**
     * Copy constructor for <code>BaseMatrix</code>. Creates a deep copy of the
    ...
    /**
     * <p>
     * Initializing constructor for bases class <code>BaseMatrix</code>. Sets
     ...
     *                                };
     * </code>
     * </pre> This array would produce a 3&times;5 matrix. Note that the given
     * argument becomes the internal representation of the matrix object. Thus,
     * the Java array <code>arrInternal</code> will be changed by the the
     * encapsulating matrix object so should no longer be referenced after
     * presenting it to this constructor.
     * </p>
     *
     * @param arrVals Java primitive array to be new internal matrix value
     * representation
     *
     * @exception IllegalArgumentException the argument is degenerate and cannot
     * represent a matrix
     *
    ...
    /**
     * Initializing constructor for class <code>BaseMatrix</code>. Sets the
     * internal matrix to the copy of the argument.
     *
