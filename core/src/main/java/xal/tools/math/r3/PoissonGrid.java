 ...
 *
 * where <italic>Val</italic> is potential value on the grid and
 * <italic>Src</italic>
...
public class PoissonGrid extends Grid implements Serializable {
    ...
    /**
     * Grid point with Neumann boundary conditions, gradient in the first
     * direction
     */
    public static final int PT_NEUMANN1 = 3;

    /**
     * Grid point with Neumann boundary conditions, gradient in the second
     * direction
     */
    public static final int PT_NEUMANN2 = 4;

    /**
     * Grid point with Neumann boundary conditions, gradient in the third
     * direction
     */
     ...
     */
    /**
     * Solve for the potential on the grid using a Gauss-Seidel relaxation
     * technique.
     *
     ...
     *
     * @exception GridException procedure did not converge to prescribed
     * residual error
     */
    ...
    public double solveCylindrical(int intIterMax, double dblErrMax) throws GridException {
        ...
        // array of grid weights
        double[] arrWts;
        // coordinates of a point on the grid
        R3 pt;
        // current grid point
        Point gpt;
     ...
     */
    /**
     * Compute and return interpolated potential at a point pt within grid
     * definition. The potential is calculated by linear interpolation of the
     * potential values at each vertex of the cell containing point pt.
     *
     * NOTES: - The grid may be in any coordinate system - The potential V must
     * be determined for all points on the grid.
     *
    ...
    /**
     * Compute and return the field in cartesian coordinates at point pt as
     * generated by the potential values on the grid.
     *
     * NOTES: - The coordinates of the grid are assumed to be (x1,x2,x3)=(x,y,z)
     * - The potential V must be determined for all points on the grid. - The
     * generated field is given by F=-grad V where V is the potential on the
     * grid.
     *
    ...
    /**
     * Compute and return the field in cylindrical coordinates at point pt as
     * generated by the potential values on the grid.
     *
     * NOTES: - The grid coordinates are assumed to be (x1,x2,x3)=(r,theta,z) -
     * The potential V must be determined for all points on the grid. - The
     * generated field is given by F=-grad V where V is the potential on the
     * grid.
     *
    ...
    /**
     * Compute and return the field in spherical coordinates at point pt as
     * generated by the potential values on the grid.
     *
     * NOTES: - The grid coordinates are assumed to be (x1,x2,x3)=(R,theta,phi)
     * - The potential V must be determined for all points on the grid. - The
     * generated field is given by F=-grad V where V is the potential on the
     * grid.
     *
    ...
    /**
     * Override base class allocator in order to allocate modified grid point
     * objects. Allocates the array of grid points for the grid.
     *
    ...
    /**
     * Override base class allocator in order to allocate modified grid cell
     * objects. Allocates the array of grid cells for the grid
     *
     * @param n1 size of the first dimension
     * @param n2 size of the second dimension
     * @param n3 size of the third dimension
     ...
     */
    /**
     * Compute the weighting coefficients of this grid for a Gauss-Seidel
     * relaxation solution technique for Poisson's equation. The coefficients
     * are determined for a cartesian coordinate system where the grid
     * dimensions are assumed (x1,x2,x3)=(x,y,z).
     *
     ...
     * Compute the weighting coefficients of this grid for a Gauss-Seidel
     * relaxation solution technique for Poisson's equation. The coefficients
     * are determined for a cylindrical coordinate system where the grid
     * dimensions are assumed (x1,x2,x3)=(r,theta,z).
     *
