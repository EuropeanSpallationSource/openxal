...
/**
 * Propagates errors from independent source variables to the specified
 * operation using standard error propagation.
 */
public class ErrorPropagator extends java.lang.Object {
     ...
     * Constructor
     *
     * @param baseOperation the operation for which the error propagation will
     * be performed
     * @param sourceVariables independent variables from which source errors are
     * to be propagated
     */
    ...
    public void setSourceVariance(final DifferentiableVariable sourceVariable, final double variance) {
        if (sourceVariable == null) {
            throw new IllegalArgumentException("ErrorPropagator: The source variable cannot be null.");
        }
     ...
     *
     * @param sourceVariances the variances to each source in the order they
     * were specified in the constructor
     */
    public void setSourceVariances(final double... sourceVariances) {
        if (sourceVariances.length != sourceVariables.length) {
            throw new IllegalArgumentException("ErrorPropagator: The count of variances: " + sourceVariances.length + " must match the count of source variables: " + sourceVariables.length);
        }
    ...
    /**
     * Calculate the operation variance propagated from the default source
     * variances
     */
    ...
    /**
     * Calculate the operation variance propagated from a common source
     * variance. Note that this assignment is only for the current calculation
     * and does change the default source variances.
     */
     ...
     * Note that this assignment is only for the current calculation and does
     * change the default source variances.
     */
    ...
    /**
     * Calculate the operation standard error propagated from the default source
     * variances
     */
