...
public class KeyValueQualifier implements Qualifier {
    ...
    /**
     * comparison operation - record's value corresponding to qualifier's key
     * must be less than the qualifier's value
     */
    public static final int COMPARE_LESS_THAN = -2;

    /**
     * comparison operation - record's value corresponding to qualifier's key
     * must be less than or equal to the qualifier's value
     */
    public static final int COMPARE_LESS_THAN_OR_EQUALS = -1;

    /**
     * comparison operation - record's value corresponding to qualifier's key
     * must be equal to the qualifier's value
     */
    public static final int COMPARE_EQUALS = 0;

    /**
     * comparison operation - record's value corresponding to qualifier's key
     * must be greater than or equal to the qualifier's value
     */
    public static final int COMPARE_GREATER_THAN_OR_EQUALS = 1;

    /**
     * comparison operation - record's value corresponding to qualifier's key
     * must be greater than the qualifier's value
     */
     ...
     * @param value the value with which to the compare the record's value for
     * the specified key
    ...
    /**
     * Determine if the specified object is a match to this qualifier's
     * criteria. The object must implement the KeyedRecord interface and its
     * value corresponding to <code>_key</code> must implement the
     * java.util.Comparable interface. See java.util.Comparable for more
     * information.
     *
