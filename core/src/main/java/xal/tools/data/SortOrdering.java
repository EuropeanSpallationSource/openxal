...
public class SortOrdering implements Comparator<KeyedRecord> {
    ...
    /**
     * Compare two records. The sort ordering compares records based on their
     * values associated with the sort ordering keys. These associated values
     * must be comparable (i.e. their class must implement the Comparable
     * interface). The comparison is made starting with the first key and
     * considers successive keys as necessary until the records can be ordered
     * or all keys are exhausted resulting in equality.
     *
     * @param record1 The first record of the comparison.
     * @param record2 The second record of the comparison.
     * @return 0 if the records are equal, negative if record2 > record1 and
     * positive if record1 > record2
     */
