...
/**
 * <p>
 * <code>DataAdaptor</code> is a generic interface to an external data source. A
 * specific data adaptor (e.g. <code>XmlDataAdaptor</code>) will implement this
 * interface to advertise its data in a generic way for reading and writing to a
 * specific data source (e.g. XML file).  <code>DataAdaptor</code> is intended to
 ...
 * receivers and writers should implement the <code>{@link DataListener}</code>
 * interface (or the <code>{@link xal.tools.data.IArchive}</code> interface for
 * model persistence).
 * </p>
 * <p>
 * A <code>DataAdaptor</code> instance is generated for each node in a
 ...
 * having a data label (see <code>{@link DataAdaptor#name()}</code>),
 * attributes, and possibly some type of string content (see
 * <code>DataAdaptor#getContent()</code> - <em>currently unimplimented</em>).
 * There are potentially child data nodes of each node (see
 * <code>{@link DataAdaptor#createChild(String)}</code> and
 * <code>{@link DataAdaptor#childAdaptor(String)}</code>).
 * <p>
 * </p>
 * Any object that is able to read and write from <code>DataAdaptor</code>
 * interfaces can implement the <code>DataListener</code> interface. The idea is
 * that such an object should know, itself, how to load and save data from this
 * interface, and expect to receive the appropriate <code>DataAdaptor</code>
 * object for doing so. Finally, note that the <code>update(DataAdaptor)</code>
 * and <code>write(DataAdaptor)</code> methods of the <code>IDataAware</code>
 * interface expect <code>IDataAdaptors</code> in different positions on the
 * data tree. This must be the case for proper logistical implementation. The
 * <code>update()</code> method expects to be synchronized to the current data
 * node while the <code>write()</code> method expects to be given the parent of
 * the data node it is to populate. See the documentation for the
 * <code>{@link DataListener}</code> interface for more information.
 *
 * @author Tom Pelaia
...
public interface DataAdaptor {
     ...
     * @return Array of double values, a <code>null</code> value is returned if
