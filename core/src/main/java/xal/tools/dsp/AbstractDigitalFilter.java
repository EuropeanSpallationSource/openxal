 ...
 * of finite order with real coefficients. Child classes can implement methods
 * for determining the filter coefficients for desired bandwidth and other
 * transfer characteristics (i.e., Butterworth, Chebychev, etc.). Once the
 * filter coefficients are determined this class contains most of the common
...
public abstract class AbstractDigitalFilter {
     ...
     *
     * @return output coefficient at time <var>iTime</var> and delay
     * <var>iDelay</var>
     */
    ...
    /**
     * Returns the number of input and output coefficients. This values is one
    ...
    /**
     * <p>
     * Return the value of the current time index. This value is the number of
     * signal values processed (by a call to
     * {@link AbstractDigitalFilter#response(double)}) since the last call to
     * {@link AbstractDigitalFilter#reset()}. After calling <code>reset()</code>
     * the returned index is zero. Thus, after calling
     * <code>response(double)</code> for the first time the returned value is 1.
     * </p>
     * <p>
     ...
     * {@link AbstractDigitalFilter#getOutputCoefficient(int, int)} the methods
     * are called <strong>before</strong> this index is updated. For example,
     * upon the first call to {@link AbstractDigitalFilter#response(double)}
    ...
    /**
     * <p>
     * Compute and return the response of the filter to the given input. The
     * returned response is assumed to be part of a train of values with depends
     * upon the previous inputs to this method. The number of previous input
     * values to which the output depends is given by the order of this filter.
     * </p>
     * <p>
     ...
     * {@link AbstractDigitalFilter#getInputCoefficient(int, int)} and
     * {@link AbstractDigitalFilter#getOutputCoefficient(int, int)} are called
     * within this method to retrieve the filter coefficients at the current
     * time index. The index used in these calls is the pre-updated value, not
     * the value after this function returns. For example, upon the first call
     * to {@link AbstractDigitalFilter#response(double)} these methods will be
     * called with the time index as 0.
     * </p>
     *
    ...
    /**
     * Convenience function for computing the response of this filter to an
     * input signal train. This method simply calls
     * {@link AbstractDigitalFilter#response(double)} sequentially by increasing
     * index for each element of the argument <var>arrTrain</var>. Thus, the
     * returned response depends upon the initial state of the filter when this
     * method is called.
     *
