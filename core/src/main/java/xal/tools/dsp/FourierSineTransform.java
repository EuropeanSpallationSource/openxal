 ...
 * the function <em>f</em> being transformed is assumed to have value zero at
 ...
 * <br>
 * <br>&nbsp;&nbsp; [<strong>f^</strong>] =
 * [<strong>K</strong>]&middot;[<strong>f</strong>]</p>
 * <br>
 * where [<strong>f^</strong>] is the vector of transformed data,
 * [<strong>K</strong>] is the real symmetric matrix kernel, and
 * [<strong>f</strong>] is data vector of input function values. The elements
 * <em>K<sub>mn</sub></em> of the matrix kernel are given by
 * <br>
 * <br>&nbsp;&nbsp;    <em>K<sub>mn</sub></em> =
 * (2/(<em>N</em>-1))<sup>&frac12;</sup>
 * sin <em>&pi;mn</em>/(<em>N</em>-1)<br>
 * <br>
 * where <em>N</em> is the size of the data vector [<strong>f</strong>], and
 * indices <em>m, n</em> range over the values 0,&hellip;,<em>N</em>. The factor
 ...
 * transform, specifically [<strong>K</strong>].[<strong>K</strong>] =
 * [<strong>I</strong>] where [<strong>I</strong>] is the identity matrix (sans
 * the initial and final 1 entry).
 * </p>
 * <p>
 * From the value of <em>K<sub>mn</sub></em> we see that the stride in
 * [<strong>f^</strong>] is 1/2<em>T</em>, where
 * <em>T</em> is the period of the interval over which <em>f</em> is defined.
 * Thus, the largest frequency we can see is (<em>N</em>-1)/2<em>T</em>. Also,
 * we see that there is no specific DC component. That is, the DC (or
 * zero-frequency) component of the signal has a projection upon all the
 * odd-frequency components in this general Fourier sine transform. Thus, it
 * cannot be identified separately as in the traditional Fourier transform.
 * </p>
 *
 * @author Christopher K. Allen
 *
