 ...
 * expensive for large signal sizes (of order <i>O</i>(<i>N</i>&sup2;)). The
 * advantage here is that we may consider arbitrary signal sizes <i>N</i> > 0.
 * That is,
 * <i>N</i> does not need to be a power of 2 as with the FFT, requiring the
 ...
 * <br>
 * where [<b>f^</b>] is the complex vector of DFT data, [<b>K</b>] is the
 * complex symmetric matrix kernel, and [<b>f</b>] is the real vector (e.i.,
 * type <code>double[]</code>) of input function values. The elements
 * <i>K<sub>mn</sub></i> of the matrix kernel are given by
 * <br>
 * <br>&nbsp;&nbsp;    <i>K<sub>mn</sub></i> =
 * <i>z<sup>-mn</sup></i>/<i>N</i><sup>&frac12;</sup> <br>
 * <br>
 * where <i>N</i> is the size of the data vector [<b>f</b>], indices <i>m, n</i>
 * range over the values 0,&hellip;,<i>N</i>-1, and <i>z</i> is the generator of
 * the transform kernel given by
 * <br>
 * <br>&nbsp;&nbsp;  <i>z</i> &equiv; <i>e<sup>i</i>2<i>&pi;</i>/<i>N</i></sup>
 * <br>
 * <br>
 * The factor 1/<i>N</i><sup>&frac12;</sup> is a normalization constant;
 * specifically, the value of the <i>L</i><sub>2</sub> norm
 ...
 * <br>
 * where the elements <i>K<sub>mn</sub></i><sup>-1</sup> of the kernel are given
 * by
 * <br>
 * <br>&nbsp;&nbsp;    <i>K<sub>mn</sub></i><sup>-1</sup> =
 * <i>z<sup>mn</sup></i>/<i>N</i><sup>&frac12;</sup> <br>
 * <br>
 * Clearly [<b>K</b><sup>-1</sup>]&middot;[<b>K</b>] = [<b>I</b>] where
 * [<b>I</b>] is the
 * <i>N</i>&times;<i>N</i> identity matrix.
 * <p>
 * From the value of <i>K<sub>mn</sub></i> and <i>z</i> it can be inferred that
 * the stride in [<b>f^</b>] is 1/<i>T</i>, where
 ...
 * the largest frequency we can see is &frac12;<i>N</i>/<i>T</i>, corresponding
 * to the discrete frequency <i>N</i>/2. Referring to the definition of
 * <i>z</i>, the positive (discrete) frequency components cover the indices
 * <i>n</i> = 0,&hellip;,<i>floor</i>(N/2) while the negative frequency
 * components are located at the indices <i>n</i> =
 * <i>floor</i>(<i>N</i>/2)+1,&hellip;,<i>N</i>-1 (in reverse order).
 * Topologically, the positive frequencies {<i>n</i>} occur for
 * <i>z<sup>n</sup></i> on the top half-plane and the negative frequencies
 * {<i>n</i>} occur for <i>z<sup>n</sup></i> on the bottom half-plane.
 * </p>
 *
 * @author Christopher K. Allen
 *
...
public class FourierExpTransform {
     ...
     * transform is identically zero, as is the <i>N<sup>th</sup></i> component.
     ...
     * The frequency <i>N</i> - 1 is actually the negative frequency -1. Thus,
     * the negative frequency -<i>n</n> is located at index
