...
/**
 * AsynchronousMessageHandler Handle dispatch messages in an asynchronous way so
 * that control is returned to the sender immediately without waiting for
 * recipients to receive their messages. Note that multiple recipients may also
...
class AsynchronousMessageHandler<T> extends MessageHandler<T> implements Serializable {
    ...
    /**
     * identifies the message handler as asynchronous overrides the abstract
     * version
     */
    @Override
