...
/**
 * SynchronousMessageHandler Handle dispatch messages in a synchronous way so
 * that control is returned to the sender only when all recipients have been
 * notified. Note that multiple recipients may be notified concurrently,
 * however.
 *
 * @author tap
 */
class SynchronousMessageHandler<T> extends MessageHandler<T> implements java.io.Serializable {
    ...
    /**
     * identifies the message handler as synchronous overrides the abstract
     * version
     */
    @Override
