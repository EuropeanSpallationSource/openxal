...
/**
 * Factory to create a complete lattice from an XAL accelerator sequence or
 * accelerator combo sequence.
 *
 * @author wdklotz
 */
...
//		STANDARD_THIN_KINDS.add("Marker");
    ...
    /**
     * Create a new instance of LatticeFactory with default element kinds. Kinds
     * of thick (and slim) elements default to
     * <code>["DH","QH","QV","PQ","SH","SV","RG","BCM"].</code> Kinds of thin
     * (and slim) elements default to
    ...
    /**
     * Concatenate two lattices and generate the Node-to-Element dictionary for
     * the resulting combined lattice. The concatenated lattice will be made of
     * clones from the lattices given as input variables.
     *
...
//		double seq_pos = sequence.getPosition();
    ...
    /**
     * Recursive walk through all accelerator sequences and subsequences,
     * filtering nodes of kind listed in 'kind' and returning a list of lattice
     * elements in 'result'.
     *
     * @param sequence the XAL accelerator sequence.
     * @param kinds list of node types (kinds) to filter to go into the lattice.
     * @param result the resulting list of lattice elements that passed the kind
     * filter.
     * @throws LatticeError
     */
     ...
     * me. This is very brittle. The order of a case statement should not
     * matter, here it is critical.
...
//		String type = node.getType();
     ...
     * Calculation stops at subsequences that have a non zero length, i.e. it is
     * assumed that a sequence with non zero length is already the sum of all
     * its subnodes and subsequences.
     *
     * @param seq the top sequence of nested accelerator sequences or a combo
     * sequence.
     */
     ...
     *
     * @param allElements the list of (@link xal.slg.Element lattice elements}
     * to be sorted.
     */
     ...
     *
     * @param verbose <code>true</code> for verbose output else
     * <code>false</code>.
     */
    ...
    /**
     * Set flag to force lattice generator to place a permanant marker in the
     * middle of every thick element.
     *
     * @param halfmag <code>true</code> yes put the middle marker (default),
     * else <code>false</code> for no middle markers. Note that the forward
     * dictionary that maps from accelerator nodes to lattice elements needs
     * these middle markers. Unless the dictionary is not needed the default
     * setting should always be accepted.
     */
