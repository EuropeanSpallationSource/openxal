 ...
 * <code>{@link #getParts()}</code>).
 * </p>
 * <p>
 * &middot; I have modified this objects so it carries the additional attribute
 * of the <em>modeling element</em> identifier. This is in contrast to the
 ...
 * <code>xal.smf</code> hardware representation subsystem. These systems should
 * be independent, able to function without each other.
 * <br/>
 * <br/>
 * &middot; I cannot really tell exactly what is happening as there was no
...
public class LatticeElement implements Comparable<LatticeElement> {
    ...
    /**
     * original index position of the hardware within its accelerator sequence -
     * this is used to sort thin elements
     */
    ...
    //
    /**
     * CKA: Modeling element identifier, which can be different that the
     * Accelerator node's ID
     */
    private String strElemId;

    /**
     * flag indicating that this is actually an "artificial" element with no
     ...
     * node entrance and exit positions are initialized using the given center
     * position and length attribute.
     *
     * @param smfNode associated hardware node
     * @param dblPosCtr center position of hardware node within accelerator
     * sequence
     * @param clsModElemType class type of the modeling element for associated
     * hardware
     * @param originalPosition index position of the hardware node within its
     * sequence (used to sort elements)
     *
     * @since Dec 8, 2014
     */
    ...
    /**
     * Initializing constructor for <code>LatticeElement</code>. The entrance
     * and exit positions are given directly for this constructor, which is
     * called only within this class. This constructor is used when splitting
     * lattice elements.
     *
     * @param smfNode associated hardware node
     * @param dblPosStart entrance location of this lattice element within
     * sequence
     * @param dblPosEnd exit location of this lattice element within sequence
     * @param clsModElemType class type of the modeling element for associated
     * hardware
     * @param originalPosition original index position of hardware node within
     * its sequence
     *
     * @since Dec 8, 2014
     */
    ...
    /**
     * Sets the (optional) string identifier for the modeling element that this
     * object will create. Typically used when splitting up modeling elements
     * associated with the proxied hardware node.
     *
    ...
    /**
     * <p>
     * Returns the length associated with the <em>hardware node</em>. This
     * length depends upon the length of the hardware node and how many times it
     * has been split to create the appropriate modeling element.
     * </p>
     * <p>
     * Note that the <em>effective</em> length of the associated hardware node
     * is used. If the hardware is a bending magnet then this is the path
     * length. If the hardware node <em>is not</em> a magnet then the physical
     * length is returned. These values are determined in the
     * <code>LatticeElement</code> constructor.
     * </p>
     *
     * @return length of this lattice element based upon hardware length and
     * splitting
     *
     * @since Dec 9, 2014
     */
    ...
    /**
     * Returns the entrance location within its parent element sequence of the
     * modeling element to be created. This value is derived from the associated
     * hardware position and the splitting of the representative lattice
     * elements.
     *
     * @return entrance position of the modeling element w.r.t. the parent
     * sequence
     *
     * @since Dec 9, 2014
     */
    ...
    /**
     * Returns the center location of the modeling element to be created. This
     * value is derived from the associated hardware position and the splitting
    ...
    /**
     * Returns the exit location of the modeling element to be created within
     * its parent accelerator sequence.
     *
    ...
    /**
     * Determines whether or not the hardware accelerator node will be
     * represented with a thin modeling element. Looks at both the <em>effective
     * length</em>
     ...
     * with respect to the axial positions. Specifically, if the entrance
     * location of this element is greater than or equal to the entrance
     * location of the given element, and the exit location of this element is
     * less than or equal to the exit location of the given element, this method
     ...
     */
    /**
     * Translates the element by the given amount along the beamline axis (of
     * which sequence it belongs).
     *
     * @param dblOffset distance by which this element is translated (either
     ...
     * </p>
     * <p>
     * The number of parts is doubled and I can't figure this out?? I assume
     * "parts" is the number of modeling elements used to represent the
     * hardware.
     * </p>
     * <p>
     * In any event, this lattice element is modified, its position now lives
     * only up to the center position of the given element.
     * </p>
     * <p>
     ...
     *
     * @return a new lattice element which is the second part of this original
     * lattice element
     *
     * @author Christopher K. Allen
     * @since Dec 4, 2014
    ...
    public LatticeElement splitElementAt(final LatticeElement elemSplitPos) {
        if (elemSplitPos.dblElemCntrPos == dblElemEntrPos || elemSplitPos.dblElemCntrPos == dblElemExitPos) {
            return null;
        }
     ...
     * object. Java reflection is used to create a new instance from the
     * element's class type. There must be a zero constructor for the element.
     * </p>
     * <p>
     * <h4>CKA Notes</h4>
     ...
     * and the elements are both thin then we look at the position index within
     * the sequence.
     *
