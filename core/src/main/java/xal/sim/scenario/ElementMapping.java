...
public abstract class ElementMapping {
     ...
     * default set for SNS
     */
    protected boolean bolSubsectionCtrOrigin = true;
    ...
    /**
     * Returns the default class type used to model accelerator sequences. This
     * value is used when none has been defined for a specific hardware sequence
     * (e.g., a super conduction cavity, coupled cavity linac, etc.).
     *
    ...
    /**
     * Creates a drift space within an RF cavity structure. Such drifts have
     * extra parameters needed to compute the probe's longitudinal phase
     * advance. These parameters are derived from the cavity's design parameters
     ...
     *
     * @throws ModelException an exception occurred while constructing the
     * element
     *
    ...
    /**
     * <p>
     * Returns modeling element class type used to model the given node. Default
     * implementation traverses a list and returns first association for the
     * given hardware type.
     * </p>
     * <p>
     * <h4>CKA NOTES:</h4>
     ...
     *
     * @return class type for the modeling element associated with the given
     * hardware
     *
    ...
    /**
     * Returns the modeling element class type used to represent the given
     * hardware accelerator sequence. Currently there are no special entry types
     * in the mapping table, it is searched just as in
     * <code>{@link #getModelElementType(AcceleratorNode)}</code> then the
     * result is cast to a an <code>IComposite</code> type. If this casting
     * fails a <code>ClassCastException</code> is thrown. However, unlike
     * <code>{@link #getModelElementType(AcceleratorNode)}</code>, if no entry
     * is found a <code>null</code> value is returned.
     *
     * @param smfSeq accelerator hardware node to be cross referenced
     *
     * @return the modeling sequence class type used to represent the given
     * accelerator node, or <code>null</code> if no class type entry is found
     *
     * @throws ClassCastException the hardware sequence mapped to an
     * incompatible model sequence element
     *
    ...
    /**
     * Removes a converter from the list. This method is useful if one wants to
     * modify a given Mapping.
     *
     * @param key
     */
    ...
    /**
     * Set flag to force lattice generator to place a permanent marker in the
     * middle of every thick element.
     *
     * @param halfmag <code>true</code> yes put the middle marker (default),
     * else <code>false</code> for no middle markers.
     */
    ...
    /**
     * Set flag to determine whether debugging information is sent to standard
     * output.
     *
     * @param bolDebug  <code>true</code> for debugging output, else
     * <code>false</code> to stop debugging output.
     */
     ...
     */
    /**
     * If the value here is <code>true</code> then marker modeling elements are
     * placed at the center of thick magnets when the model lattice is created.
     *
     * @return the flag to force lattice generator to place a permanent marker
     * in the middle of every thick element.
     */
    ...
    /**
     * Get the debugging flag. If <code>true</code> then debugging information
     * is being sent to the standard output.
     *
     * @return  <code>true</code> if debugging information is being sent to
     * standard output, <code>false</code> when in normal operation.
     */
    ...
    /**
     * Returns whether or the origin of the axis of subsection is at the center
     * of the sequence, normally it is located at the entrance of the sequence.
     *
