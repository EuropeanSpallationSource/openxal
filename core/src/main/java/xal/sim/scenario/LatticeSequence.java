...
/**
 * <p>
 * The Open XAL online model <em>lattice generator</em> is primarily contained
 * here.
 * </p>
 * <p>
 * Given a valid <code>ElementMapping</code> object, <code>AcceleratorSeq</code>
 * object, and <code>SynchronizationManager</code> for the online model, the
 * <code>LatticeSequence</code> will create an online model <code>Lattice</code>
 * object. This object is used by a <code>ScenarioGenerator</code> to create a
 * new <code>Scenario</code> object.
 * </p>
 * <p>
 * Thus, the actual "lattice generation" is done here. The peripheral resources
 * such as hardware node to model element mappings and synchronization managers
 * are created outside this class.
 * </p>
 *
 *
 * @author Christopher K. Allen
...
public class LatticeSequence extends LatticeElement implements Iterable<LatticeElement> {
     ...
     */
    /**
     * container list of lattice element associations (some may be lattice
     * sequences)
     */
    ...
    //
    /**
     * The set of hardware node to element mapping associations used to create
     * model elements
     */
     ...
     * <code>LatticeSequence</code> object for the given accelerator sequence
     * under the assumption that that sequence is the top level (i.e., not a
     * sub-sequence).
     * </p>
     * <p>
     * <h4>NOTES</h4>
     * &middot; The lattice element position and index parameters for the base
     * class are both set to zero since this is the root of the lattice
     * structure.
     * </p>
     *
     * @param smfSeqRoot top level associated hardware accelerator sequence
     * @param mapNodeToElem the mapping of SMF hardware nodes to modeling
     * element class types
     *
    ...
    /**
     * <p>
     * Internal constructor for lattice sequences. This constructor is called
     * when making subsequences within the root or parent sequences. That is,
     * this sequence is intended to be the child of the provided lattice
     * sequence.
     * </p>
     * <p>
     * This constructor needs the additional arguments of lattice position and
     * index within its parent sequence, which are necessary in the <em>lattice
     * element</em> creations. The lattice elements use these values to order
     * themselves, split themselves, and create drifts between them.
     * </p>
     * <p>
     * We need to use a constructor when instantiating new lattice sequences in
     * order to call the base class constructor (i.e.,
     * <code>LatticeElement{@link #addLatticeElement(LatticeElement)}</code> to
     * pass in the appropriate accelerator hardware and hardware to element
     * mappings.
     * </p>
     *
     * @param latSeqParent the parent lattice of this lattice
     * @param smfSeqChild the associated hardware accelerator sequence for which
     * this lattice is being created, i.e., the associated child sequence for
     * the given parent lattice sequence
     * @param dblPos position of the accelerator sequence within parent (if
     * applicable)
     * @param indOrgPos index of the sequence within its parent sequence (if
     * applicable)
     *
     ...
     */
    /**
     * Indicates whether or not the associated hardware accelerator sequence is
     * an RF cavity structure. Such structures are derived from the base class
     * <code>{@link xal.smf.impl.RfCavity}</code>.
     *
     ...
     *
     * @return  <code>true</code> if the axis origin is at the center of the
     * sequence, <code>false</code> otherwise (likely at the sequence entrance)
     *
     ...
     *
     * @param smfSeq accelerator sequence hardware object to be looked up in the
     * map
     *
    ...
    /**
     * Returns the map of SMF hardware nodes to online modeling elements, or
     * more specifically, modeling element class types. This map is used when
     * creating a modeling element for any given hardware node.
     *
     ...
     */
    /**
     * Creates a new model lattice object according to the configuration of this
     * lattice sequence. The given synchronization manager is populated with
     * synchronization associations for the returned model lattice. However, the
     * synchronization manager is still unbound to any scenario.
     *
     * @param mgrSync synchronization manager to receive synchronization
     * associations for the model elements in the returned model lattice
     *
     ...
     */
    /**
     * Iterates element by element through the element lattice from head to
     * tail.
     *
     ...
     * <code>{@link LatticeElement#getHardwareNode()}</code> method so that it
     * down casts the result to the tighter type of <code>AcceleratorSeq</code>
     * rather than <code>AcceleratorNode</code>. This class can be instantiated
     * only with a <code>AcceleratorSeq</code> derived hardware node, we are
     * just enforcing it here.
     *
     * @return the accelerator sequence hardware associated with this lattice
     * sequence object
     *
    ...
    /**
     * <p>
     * Creates and initializes a new modeling composite sequence represented by
     * this object. Java reflection is used to create a new instance from the
     * sequence's class type. There must be a zero constructor for the element.
     * </p>
     * <p>
     * <h4>CKA Notes</h4>
     * &middot; The parameter initialization is done by calling the
     * <code>{@link IComponent#initializeFrom(LatticeElement)}</code>. This
     * design effectively couples the <code>xal.model</code> online model
     * subsystem to the <code>xal.smf</code> hardware representation subsystem.
     * These systems should be independent, able to function without each other.
     * </p>
     *
     * @return a new modeling sequence for the hardware proxied by this object
     *
     * @throws ModelException Java reflection threw an
     ...
     */
    /**
     * Builds an online model element sequence for the accelerator sequence
     * provided to the public constructor of this class. This is done by calling
     * the private methods
     * {@link #populateLatticeSeq()}, {@link #splitSequenceElements()}, and
     * {@link #createModelElements(SynchronizationManager)} in that order. When
     * the actual modeling elements are instantiated (via
     * {@link #createModelElements(SynchronizationManager)}) they are connected
     * to the given synchronization manager.
     *
     * @param mgrSync synchronization manager to receive synchronization
     * associations for the new created model elements
     *
    ...
    protected Lattice createParentLattice() {
        ...
        String strComment = "";
        if (smfAccel != null) {
            strComment += "Accelerator ID:" + smfAccel.getId() + ", ";
        }
    ...
    /**
     * <p>
     * CKA : I have modified the original method so that it is now recursive. It
     * calls itself whenever it finds a hardware node within the given
     * accelerator sequence which is actually another accelerator sequence. In
     * that case another lattice sequence is created (which is also a lattice
     * element) and added to the current lattice sequence as a lattice element.
     * </p>
     * <p>
     * Collects all elements in the smfSequence into a single list, recording
     ...
     * </p>
     * <p>
     * If <code>bolDivMags</code> is set, also adds a center marker for each
     * magnet.
     * </p>
     *
    ...
    /**
     * If this sequence has its axial origin at the center of the sequence then
     * all the elements are translated such that the new origin is at the
    ...
    private void enforceAxisOrigin() {
        ...
        for (LatticeElement el : this) {
            ...
            if (el.getEndPosition() > dblEndSeqChild) {
                dblEndSeqChild = el.getEndPosition();
            }
        }
    ...
    /**
     * Looks for all the grand children of this sequence who are in direct
     * ownership of this sequence. The method pushes them down the family tree
     * until they are living with their direct parents. This is also a recursive
     * method where each sequence pushes the orphaned elements down one level
     * until the recursion stops at the last lattice sequence in a branch.
     *
     ...
     * sequence according to the natural ordering defined in
     * <code>LatticeElement</code>.
     * </p>
     * <p>
     * Note that the natural ordering is defined by the
     * <code>{@link LatticeElement#compareTo(LatticeElement)}</code> method
     * required by the <code>{@link java.lang.Comparable}</code> interface.
     * </p>
     *
    ...
    /**
     * <p>
     * Splits all thick elements by thin ones, keeping the order.</p>
     * <p>
     * The method also checks if two thick elements cover each other, which
     * should not happen.</p>
     * <p>
     * Uses a simple scan line algorithm.</p>
     * <p>
     * <h4>CKA NOTES</h4>
     * &middot; The change to this sequence and its child lattice elements is
     * irreversible. Once lattice elements are split (via the
     * <code>LatticeElement{@link #splitElementAt(LatticeElement)}</code>
     * method), the element cannot be reconstituted.
     * </p>
     *
     * @throws ModelException encountered a nonzero intersection of two thick
     * elements (cannot process this)
     */
            ...
            if (lemLastThick instanceof LatticeSequence) {
                ...
                if (mapNodeToMdl.isDebugging()) {
                    ...
                    if (elemSplitPartEnd != null) {
                        ostrDebug.println("\t" + elemSplitPartEnd.toString());
                    }
                }
    ...
    /**
     * <p>
     * Visits each element and invokes conversion on it, using element mapper on
     * it.</p>
     * <p>
     * Hooks synchronization manager to each element.</p>
     * <p>
     * Adds drifts between the elements.</p>
     * <p>
     * <h4>CKA NOTES</h4>
     * &middot; Here we differentiate between default drift spaces and RF cavity
     * drift spaces depending upon whether or not this sequence represents an RF
     * cavity strucure.
     * <br/>
     * <br/>
     * &middot; This is a recursive algorithm. It calls itself whenever it finds
     * a lattice element that is actually a lattice sequence.
     * </p>
     *
     * @param mgrSync manager used to synchronize modeling elements with live
     * hardware
     * @param mapNodeToMdlCls the mapping association between hardware node
     * types and modeling element clas types
     *
    ...
    //
    /**
     * Adds a <code>LatticeElement</code> association class to the tail of this
     * sequence of lattices elements.
     *
     * @param lem lattice element association class to become element in this
     * sequence
     *
     * @author Christopher K. Allen
     * @since Dec 9, 2014
    ...
    /**
     * Removes all the elements in the given set of elements from the contained
     * set of lattice element in this sequence;
     *
    ...
    /**
     * Called by <code>{@link #splitElements(List)}</code> to add the given
     * lattice element into the given list of split elements. The given element
     * must meet 1 of 2 criteria: 1) it has finite length or 2) it represents no
     * more than one modeling element. Otherwise the operation is rejected.
     *
    ...
    /**
     * Creates a new drift space of the appropriate length and type (w.r.t. the
     * hardware that this lattice sequence represents) and appends it to the end
     * of the given modeling composite structure.
     *
