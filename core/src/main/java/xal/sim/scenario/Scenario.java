...
public class Scenario {
    ...
    /**
     * Synchronization manager constant for synchronization only to live magnet
     * values
     */
    ...
    /**
     * Creates a new Scenario for the supplied accelerator sequence and element
     * mapping.
     *
     * @param smfSeq the accelerator sequence to build a scenario for
     * @param mapNodeToElem the element mapping to build a scenario with
     * @return a new model <code>Scenario</code> for the supplied accelerator
     * sequence
    ...
    /**
     * Synchronization manager synchronizing the model lattice parameters to the
     * synchronization source
     */
    private final SynchronizationManager mgrSync;

    /**
     * Back reference to the hardware that this scenario model - CKA: I really
     ...
     *
     * @throws SynchronizationException if an error is encountered reading a
     * data source
     */
    ...
    /**
     * Synchronizes each lattice element from the cache and applies the whatif
     * model inputs.
     *
     * @throws SynchronizationException if an error is encountered reading a
     * data source
     ...
     *
     * @throws ModelException if the node is not found, or no elements are
     * mapped to it
     */
     ...
     * id. By default the model will stop propagation <strong>AFTER</strong>
     * this node. This behavior can be changed in the Tracker ( see
    ...
    /**
     * Sets the model to stop propagation (by default) <strong>after</strong>
     * the specified IComponent. If you don't have a reference to a component
     * but do have an AcceleratorNode, use setStopNode(String). This "stop
     * after" behavior can be changed in the Tracker ( see
     * setStopNodeInclusive(boolean) method.
     *
    ...
    /**
     * Convert the position of a location in the sequence to a position in a
     * trajectory due to a start element offset specified in the scenario.
     *
     * @param positionInSequence The position of a location in this scenario's
     * sequence
     * @return the corresponding position relative to this scenario's starting
     * element
     */
     ...
     * @throws IllegalStateException if the lattice or probe is not properly
     * initialized
     */
    ...
    /**
     * Returns the lattice. NOTE: I (Craig M) don't like this here. I only added
     * it so that I could keep ModelProxy working as it presently does. Let's
     * figure out a way to change it. It seems that the only thing that gets the
     * Lattice from ModelProxy is OrbitDisplay2. If it were changed to use
     * Scenario, then this method could be removed!
     *
     * @return the Lattice
     */
     ...
     *
     * &middot; The type of the object returned is actually
     * <code>Trajectory&lt;?&gt;</code> since the actual type of the trajectory
     * is not known. Any type of probe may be used to run the scenario.
     * <br>
     * <br>
     * &middot; This is simply a convenient way to avoid the clumsy Java type
     * casting, however, it is essentially the same thing.
     * <br>
     * <br>
