 ...
 * methods to input data to the model, to generate the lattice from an
 * {@link xal.smf.AcceleratorSeq accelerator sequence}, to run the model, to
 * synchronize the model with the real accelerator, and to extract results from
 * a model run.
 *
 * @author wdklotz
 */
public class ModelProxy {
    ...
    /**
     * Create a new (empty) model proxy. Per default this proxy will be
     * synchronized by channel access.
     */
    public ModelProxy() {
        this(PARAMSRC_LIVE);
    }
     ...
     *
     * @param paramSrc an integer to indicate where to synchronize from. If
     * <code>true</code> the proxy will use channel access to read settings of
     * lattice elements, if <code>false</code> the element settings will be
     * taken from design values defined in the accelerator database.
     */
    ...
    /**
     * Run the model. The probe will be propagated through the lattice. All
     * registered ModelProxyListeners will be notified by calling
     ...
     * <code>ModelProxyListener.MISSING_INPUT</code>. If neither of these values
     ...
     * resets the probe to its initial values. All registered
     * ModelProxyListeners will be notified by:
     *
    ...
    // -------------------------- setter members ------------------------------------------
    /**
     * Setter for the {@link xal.smf.AcceleratorSeq accelerator sequence}
     * property. As a result of calling this member a new Scenario will be
     * generated. In Addition all registered ModelproxyListeners will be
     * notified by {@link xal.sim.mpx.sns.xal.model.mpx.ModelProxyListener#accelSequenceChanged
...
/*            try {
     ...
     * <code>DIAGNOSTIC_PROBE</code>, <code>ENSEMBLE_PROBE</code>,
     * <code>ENVELOPE_PROBE</code>, <code>PARTICLE_PROBE</code>
     *
    ...
    /**
     * Getter for the on-line-model {@link xal.model.probe.Probe probe}
     * property.
     *
     * @return the on-line-model probe. If probe is not set yet
     ...
     *
     * @return the opened probe file. If file is not set yet <code>null</code>
     * is returned.
     */
     ...
     * Getter for the {@link xal.model.Lattice on-line-model lattice} property.
     *
     * @return the on-line-model lattice. If lattice is not set yet
     * <code>null</code> is returned.
     */
     ...
     *
     * @return the DOM of the probe. If not set yet <code>null</code> is
     * returned.
     */
     ...
     * {@link xal.smf.AcceleratorNode accelerator node}.
     *
    ...
    /**
     * Check whether the model proxy has a valid lattice and take actions. If
     * the lattice is not valid all ModelProxyListeners will be notified by
     * calling
    ...
    /**
     * Check wether the model proxy has a valid probe and take actions. If the
     * probe is not valid all ModelProxyListeners will be notified by calling
     * {@link xal.sim.mpx.sns.xal.model.mpx.ModelProxyListener#missingInputToRunModel missingInputToRunModel}
     * and and an {@link java.lang.Error} will be thrown.
     *
     * @throws Error
     ...
     *
     * @return <code>true</code> if the model has a valid lattice ,
     ...
     *
     * @return <code>true</code> if the model has a valid probe ,
     * <code>false</code> if not.
     */
