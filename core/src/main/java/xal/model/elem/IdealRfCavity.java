 ...
 * cavity drifts. The types and parameters of the internal elements define the
 * operation (and configuration) of the cavity.
 * </p>
 * <p>
 * The propagation is done via the base class <code>ElementSeq</code> which just
...
public class IdealRfCavity extends ElementSeq implements IRfCavity {
     ...
     */
    /**
     * The current operating phase with respect to the arriving particle (in
     * radians)
     */
    private double dblPhase;

    /**
     * The amplitude of the RF signal at the cavity RF window, i.e., the
     * klystron amplitude (in Volts)
     */
    ...
    /**
     * The mode constant (1/2 the mode number) of the cavity which we are
     * exciting
     */
    ...
    /**
     * Get the amplitude of the RF signal feeding the cavity. Specifically, the
     * voltage of the RF at the cavity RF window.
     *
    ...
    /**
     * Sets the amplitude of the RF signal feeding the cavity. Specifically, the
     * voltage of the RF at the cavity RF window. This method updates the
     * voltage for all gaps in the cavity.
     *
     ...
     * element. The SMF node is taken from the proxy then queried directly. Eh,
     * I don't like this.
     *
     * @see
     * xal.model.elem.ElementSeq#initializeFrom(xal.sim.scenario.LatticeElement)
     *
    ...
    /**
     * <p>
     * Sets the probes longitudinal phase to the phase of this cavity upon
     * entrance. Then we propagate the probe through the composite structure as
     * usual by calling the base class <code>propagate</code> method.
     * </p>
     * <p>
     * It is unnecessary to override the
     * <code>{@link #propagate(IProbe, double)}</code> method since that method
     * simply defers to the <code>{@link #propagate(IProbe)}</code> method
     * ignoring the position parameter.
     * </p>
     *
     ...
     * various beam exit times and their effect.
     * </p>
     * <p>
     * It is unnecessary to override the
     * <code>{@link #backPropagate(IProbe, double)}</code> method since that
     * method simply defers to the <code>{@link #backPropagate(IProbe)}</code>
     * method ignoring the position parameter.
     * </p>
     *
     ...
     * exposes the <code>IRfCavityCell</code> interface. If so, then it is an
     * accelerating cell within this cavity and we need to set its index within
     * the cavity and the cavity structure mode constant for the cell. Together
     * these parameters allow the cavity cell to adjust its field spatially in
     * order to account for its position in the cavity and the operating mode
     * field structure.
     *
    ...
    /**
     * Compute the indices of the component cavity cells and distribute the
     * values across all the children. The indices are computed according to the
     * cell order, its position within a cell bank (e.g., and end cell), and its
     * position within the entire cavity (e.g., the first cell).
     *
