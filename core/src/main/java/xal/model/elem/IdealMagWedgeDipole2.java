...
public class IdealMagWedgeDipole2 extends ElectromagnetSeq {
    ...
    /**
     * Overrides the default <code>setHardwareNodeId(String)</code> method for
     * the <code>ElementSeq</code> base class so we can set the hardware node
     * IDs for the composite elements.
     *
...
//        this.getMagBody().setPosition(dblPos);
    ...
    /**
     * Set the physical length of the bending dipole magnet itself. The design
     * path length is generally larger than this value because of the curvature.
     *
     ...
     * {@link IdealMagSectorDipole2#getFieldIndex()}), and <em>dB/dR</em> is the
     ...
     */
    /**
     * This method overrides the <code>ElementSeq</code> base class
     * implementation to return an independent property for this element. This
     * property is set upon initialization since, although this element is a
     * sequence, it represents a single hardware node. Thus, the position is a
     * hardware property, and not a dynamic property of the sequence.
     *
...
//        return this.getMagBody().getPosition();
    ...
    /**
     * Override the method in base class <code>ElementSeq</code> to return an
     * independent property for this element. Since this class represents a
     * single hardware node it has a length that does not change.
     *
