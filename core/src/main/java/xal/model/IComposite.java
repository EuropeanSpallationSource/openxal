 ...
 * build from <code>IElement</code> objects and other composites exposing the
 * <code>IComposite</code> interface. Both interfaces are derived from the base
 * interface <code>IComponent</code>.
 *
...
public interface IComposite extends IComponent, Iterable<IComponent> {
 ...
 /*
     ...
     */
    /**
     * Return iterator over direct descendants only of this composite element in
     * sequence.
     *
    ...
    /**
     * Returns iterator over <strong>all</strong> the components in this
     * composite element in proper sequence. Parent objects should be returned
     * first than all the children. This would be the order in which the probe
    ...
    /**
     * Return the number of direct descendants in this composite. That is, any
     * child composite nodes only get a single count.
     *
     ...
     */
    /**
     * Propagates the Probe object through this composite element sequentially
     * element by element.
     *
     * <p>
     * <strong>NOTES</strong>: CKA
     * <br>
     * &middot; This looks to be redundant! It is overriding the same method in
     * the super interface <code>IComponent</code>.
     * </p>
     *
     ...
     * parameters must be computed upon demand due to dynamic changes in the
     * composite. In order to avoid expensive re-iteration through large
     * composite structures, by employing this method a state condition can be
     * set up so that the dependent parameters are only recomputed when the
     * composite changes.
     * </p>
     * <p>
     * <ul>
     * <li>Implementing classes should call this method whenever the composite
     * changes, in particular whenever
     * <code>{@link #addChild(IComponent)}</code> and
     * <code>{@link #remove(IComponent)}</code> are called.
     * <li>They respond to this call by setting a dirty flag and calls the same
     * method on their parent object.
     * <li>If the dirty flag is set then all dependent parameters should be
     * recomputed whenever one is requested (after which the flag can be
     * cleared).
     * </ul>
     * </p>
     *
