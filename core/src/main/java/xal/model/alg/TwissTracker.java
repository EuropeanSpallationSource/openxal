...
/**
 * <p>
 * Tracking algorithm for <code>TwissProbe</code> objects. The
 * <code>TwissProbe</code>'s primary state object, a
 * <code>BunchDescriptor</code> object containing three sets of Twiss parameters
 * (one for each phase plane), is advanced using the linear dynamics portion of
 * any beamline element (<code>IElement</code> exposing object) Moreover,
 ...
 * <h3>NOTES:</h3>
 * <p>
 * &middot; There is necessarily no coupling between the phase planes from
...
public class TwissTracker extends Tracker {
     ...
     *
     * NOTE: If set, the dynamics will no longer be consistent since the
     ...
     */
    /**
     * Returns the element subsection length (in meters) that the probe may be
     ...
     * <code>IdealMagQuad</code> element. The returned matrix is the transfer
     * matrix for the <code>IdealMagQuadrupole</code> element augmented with any
     * special processing.
     *
     * The argument <code>matPhi</code> is actually the transfer matrix of the
     * argument <code>elem</code> as computed by the element for some given
     * length. However, at this point it has already been computed once and we
     * avoid recomputing <code>matPhi</code> simply by providing it as an
     * argument.
     *
     * NOTES by H. SAKO put alignment error in sigma matrix
     *
    ...
    /**
     * Method to handle exceptional case of propagation through an
     * <code>IdealRfGap</code> element. The returned matrix is the correlation
     * matrix augmented with any special processing.
     *
     ...
     *
     * NOTES by H. SAKO Increase emittance using same (nonlinear) procedure on
     * the second moments as in Trace3D.
     *
     * NOTES by C.K. Allen - Although not as obvious, the procedure is the same
     * as that for the correlation matrix case (<code>EnvelopeProbe</code>)
     * objects. The <x'x'> element is modified by the formula
     *
     * <x'x'> = <x'x'> + kx*<xx>
     *
     * where kx is the phase spread. Of course there are simular equations for
     * the other phase planes. In this case we must adjust the three Twiss
     * parameters (a,b,e) = (<xx'>/e,<xx>/e,<xx><x'x'>-<xx'>^2).
     *
     ...
     *
     * @return array of Twiss parameters which include emittance growth from RF
     * gap
     *
     * @author Hiroyuki Sako
     * @author Christopher K. Allen
     ...
     * parameters, they do not need to describe the matched envelope. Likewise
     * with the given transfer matrix, it does not need to be a full turn
     * matrix, or the matrix of a periodic cell. The returned values are
     ...
     *
     * CKA NOTES: - This method will only work correctly for a beam that is
    ...
    /**
     * Appears to be a 2nd-order approximation of something in Appendix F of the
     * Trace3D manual.
     *
     * @param probe beam probe with finite phase spread
     * @param elem RF Gap producing the emittance increase
