 ...
 * Element-Algorithm-Probe design pattern, the <code>IElement</code> exposed
 * objects may interact with physical aspects of the particle beam which are
 * expressed by the <code>IProbe</code> interface.
 * <p>
 * These interactions between <code>IElement</code> objects and
 * <code>IProbe</code> objects are defined by dynamics objects exposing the
 * <code>IAlgorithm</code> interface.
 *
...
public interface IElement extends IComponent {
    ...
    /**
     * <p>
     * Returns the longitudinal phase advance of the given probe with respect to
     * the RF phase while propagation through this element section. Typically
     * used to account for phase delay/advance in cavities incurred due to
