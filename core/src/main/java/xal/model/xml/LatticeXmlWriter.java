 ...
 * by {@link LatticeXmlParser}. Calling either the static method writeXml, or
 * the instance method writeLatticeToFile with a <code>Lattice</code> object and
...
public class LatticeXmlWriter {
    ...
    /**
     * Used to start traversing the Lattice hierarchy. Adds a top level node to
     * the XML document representing the Lattice. Adds optional attributes
     ...
     * be improved in conjunction with that LatticeXmlParser code that populates
     * the comment String field. Currently, simply punts if there are not 3
     * delimited tokens, and assumes they are in the same order as written by
     * the parser. Clearly, this assumes the comment field has been populated by
     * the parser and not set by some other method, in which case these
     ...
     * XML elements for each declared bean property. The default bean
     * introspection mechanism is employed to determine the property names,
     * types, and read methods. To customize the properties for an
     * <code>Element</code> subclass, create a <code>BeanInfo</code> class that
     * defines the public bean properties for the Element subclass. Any
     * exception encountered during the introspection process cases the System
     * to exit. This may not be the appropriate behavior, but seems like a
