...
public class BatchConnectionTest {
    ...
    public final class ConnectionMonitor implements ConnectionListener {
         ...
         * <br>
         * &nbsp; &sdot; We remove ourself from the channel's set of connection
         * listeners
         * <br>
         ...
         * the channel's set of connection listeners. We should have been
         * removed when the connection request was fulfilled. However, something
         * wrong could have happened.
         *
         * @since Feb 4, 2011
     ...
     */
    /**
     * Creates a new <code>BatchConnectionTest</code> object for the given
     * accelerator device.
     *
     ...
     * off the checking methods
     * (<code>{@link #testConnection(Class, double)}</code> and
     * <code>{@link #testConnection(Collection, double)}</code>) perform no
     * operation and return a value of <code>true</code>.
     *
     * @param bolChecking if <code>true</code> then connection checking
     * continues as normal if <code>false</code> then connection checking is
     * turned off regardless
     *
     * @author Christopher K. Allen
     * @since Mar 28, 2011
     */
    ...
    /**
     *
     * Test the given (implied) SCADA class for channel connectivity of . its
     * SCADA fields All the channels for the set of PVs are identified (i.e.,
     * read back, set, etc.) then the connections are tested in batch mode,
     * rather than serially. Thus, the given timeout until failure applies to
    ...
    /**
     * Test the given set of PV field descriptors for channel connectivity. All
     * the channels for the set of PVs are identified (i.e., read back, set,
     * etc.) then the connections are tested in batch mode, rather than
     * serially. Thus, the given timeout until failure applies to all the
     * channels at once.
     *
     * @param setFds set of field descriptors
     * @param dblTmOut timeout before connectivity test fails (in seconds)
     *
     * @return  <code>true</code> if all channels described in the list are
     * connected, <code>false</code> otherwise
     *
     * @throws BadChannelException bad channel handle, no channel bound to it
     * @throws IllegalArgumentException the given collection of descriptors is
     * empty
     *
     * @author Christopher K. Allen
     * @since Mar 11, 2011
    ...
    /**
     * Returns a copy of the current set of channels that passed the
     * connectivity test (even if some channels in the original batch failed).
     *
     * @return (copy) set channels from the original batch that we able to
     * connect
     *
     * @author Christopher K. Allen
     * @since Mar 11, 2011
    ...
    /**
     * <p>
     * Add the given channel to the list of channels pending connectivity test.
     * </p>
     * <h3>NOTE:</h3>
     * <p>
     * &mdot; Only channel that are not previously connected are added to the
     * list. That is, if the <code>@link Channel#isConnected}</code> returns
     * true then the channel is not added.
     * </p>
     *
