...
public class AcceleratorSeqCombo extends AcceleratorSeq {
    ...
    /**
     * Instantiate a an AcceleratorSeqCombo or a subclass depending on whether
     * the sequences form a ring.
     *
     * @param strID The identifier of the new combo sequence
     * @param sequences The sequences to flatten into a combo sequence
     * @return a new AcceleratorSeqCombo instance if the sequences do not form a
     * ring and a Ring if they do
     */
    ...
    /**
     * Get an instance of a non-cycling combo sequence which starts and ends
     * between the specified sequences inclusively.
     *
     ...
     * @return a combo sequence ranging from the first sequence to the last
     * sequence or null if none can be found
     */
    ...
    /**
     * Get the list of all non-cycling combo sequences which start and end
     * between the specified sequences inclusively.
     *
     * @param comboID unique ID to assign to the new combo sequence
     * @param startSequence first sequence in combo
     * @param endSequence last sequence in combo
     * @return list of combo sequences ranging from the first sequence to the
     * last sequence
     */
    ...
    /**
     * Get the list of all non-cycling sequence chains which extend the
     * specified sequence chain back to the terminal sequence.
     *
     * @param terminalSequence sequence back which to extend the chains
     * @param sequenceChain initial chain of sequences to extend
     * @return all non-cycling sequence chains extending the given sequence
     * chain to the terminal sequence
     */
     ...
     *
     * @param adaptor the adaptor to which to write out this combo sequence's
     * definition.
     */
    @Override
    ...
    /**
     * get the position of a node in the sequence, including the extra length of
     * the sequence starting position itself
     *
     * Note: this way could also be done in the parent AcceleratorSeq class, be
     * we choose to use a more efficient scheme there, specific to primary
     * sequences.
     *
    ...
    public double getPosition(final AcceleratorNode node) {
        ...
        if (arrNodes == null || arrNodes.isEmpty()) {
            return dblPos + node.dblPos;
        }
    ...
    /**
     * Search deeply for and get the node with the specified id. Overrides the
     * inherited method to search down the path of the constituent sequences.
     *
    ...
    /**
     * Shallow fetch of nodes. Override the AcceleratorSeq version to only
     * return the list of nodes which are a union of those nodes which belong to
     * the base constituent sub-sequences.
     */
    @Override
    ...
    /**
     * Shallow fetch of sequences. Override the AcceleratorSeq version to only
     * return the list of sequences which are a union of those nodes which
     * belong to the base constituent sub-sequences.
     */
    @Override
     ...
     *
     * @param qualifier The qualifier to restrict which constituents are
     * returned
    ...
    /**
     * Generate the constituent sequences looking deeply even if the combo
    ...
    /**
     * Get the constituent sequences looking deeply even if the combo sequences
     * are nested in many layers.
     *
     * @return the list of constituent sequences that make the combo sequence
     */
    ...
    /**
     * Get the constituent sequences looking deeply even if the combo sequences
     * are nested in many layers. Use the qualifier to limit the constituent
     * sequences to only those which match the qualifier.
     *
     * @param qualifier The qualifier for matching which constituents get
     * returned
     * @return the list of qualified constituent sequences that make the combo
     * sequence
     */
    ...
    /**
     * Get all nodes including this sequence and constituent sequences and all
     * of their children.
     *
    ...
    /**
     * Fetch all nodes looking deeply through nested child sequences. Filter out
    ...
    /**
     * Override the equals method to return true if and only if the two combo
     * sequences have equal IDs, are both combo sequences and have equal
     * constituents.
     *
    ...
    public boolean equals(final Object sequence) {
        ...
        if (!compSequence.getId().equals(getId())) {
            return false;
        }
    ...
    public ApertureProfile getAperProfile() {
        List<AcceleratorNode> nodes = new ArrayList<>();
        nodes.addAll(getConstituents());
        return getAperProfile(nodes);
    }
}
