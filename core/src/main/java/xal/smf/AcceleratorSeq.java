...
public class AcceleratorSeq extends AcceleratorNode implements DataListener {
    ...
    private static final String SEQUENCE_LBL = "sequence";

    /**
     * indicates the node type as being a sequence
     */
    public static final String TYPE = SEQUENCE_LBL;
    ...
    public void update(final DataAdaptor adaptor) throws NumberFormatException {
        ...
        final List<DataAdaptor> sequenceAdaptors = adaptor.childAdaptors(SEQUENCE_LBL);
        for (final DataAdaptor sequenceAdaptor : sequenceAdaptors) {
            try {
                if (sequenceAdaptor.hasAttribute(EXCLUDE_ATTR) && sequenceAdaptor.booleanValue(EXCLUDE_ATTR)) {
        ...
        for (final DataAdaptor nodeAdaptor : nodeAdaptors) {
            try {
                if (nodeAdaptor.hasAttribute(EXCLUDE_ATTR) && nodeAdaptor.booleanValue(EXCLUDE_ATTR)) {
    ...
    /**
     * write this sequence's child accelerator nodes deeply traversing each
     * branch. Some applications call this method since Combo sequence overrides
     * the write method to provide only the constituents.
     */
    ...
    /**
     * Get the ID of this sequence. Subclasses which represent combinations of
     * sequences should override this method to return the ID of the first
     * sequence in the combination.
     *
     ...
     */
    public int getIndexOfNode(final AcceleratorNode child) {
        return getNodes().indexOf(child);
    }
    ...
    /**
     * Find the index to insert the node in increasing order of position. For
     * efficiency, start comparing with the last node since they are likely to
     * roughly be added in order.
     */
    ...
    /**
     * Method to move a node from one sequence to this sequence warning be
     * careful - need to check node position when moving it about!!
     */
    ...
    /**
     * Method to add a node from to this sequence the node is also kept in its
     * original sequence. This is used when concatenating nodes together into a
     * new sequence.
     */
     ...
     * @return true if the node is a shallow child of this sequence and false if
     * not
     */
     ...
     * @return the node corresponding to the requested id or null if no such
     * node is found.
     */
    ...
    /**
     * Filter nodes from the source list which can be cast to the specified
     * result class.
     *
     * @param resultClass filters the returned nodes to those which can be cast
     * to this class
    ...
    public static <S extends AcceleratorNode, N extends S> List<N> filterNodesByClass(final Class<N> resultClass, final List<S> sourceNodes) {
        // returned list
        final List<N> matchedNodes = new ArrayList<>();

        for (final S node : sourceNodes) {
            if (resultClass.isInstance(node)) {
                // the cast is only for compile since generics are stripped at runtime
                matchedNodes.add((N) node);
            }
        }
     ...
     *
     * @param resultClass filters the returned nodes to those which can be cast
     * to this class (assumes AcceleratorNode if null)
     * @param sourceNodes the list of nodes to filter
     * @param qualifier the qualifier used to filter the nodes
     * @return the list of nodes matching the qualifier criteria
     */
    public static <S extends AcceleratorNode, N extends S> List<N> getNodesOfClassWithQualifier(final Class<N> resultClass, final List<S> sourceNodes, final TypeQualifier qualifier) {
    ...
    // we do check the class cast, but the compiler has no way of knowing
    @SuppressWarnings("unchecked")
    public static <S extends AcceleratorNode, N extends S> List<N> appendNodesOfClassWithQualifier(final Class<N> resultClass, final List<N> matchedNodes, final List<S> sourceNodes, final TypeQualifier qualifier) {
     ...
     * @return the list of nodes matching the qualifier criteria
     */
    public static <S extends AcceleratorNode, N extends S> List<N> appendNodesWithQualifier(final List<N> matchedNodes, final List<S> sourceNodes, final TypeQualifier qualifier) {
     ...
     * @return the list of nodes matching the qualifier criteria
     */
    public <S extends AcceleratorNode, N extends S> List<N> getNodesOfClassWithQualifier(final Class<N> resultClass, final TypeQualifier qualifier) {
     ...
     * @return the list of nodes matching the qualifier criteria
     */
    public <S extends AcceleratorNode, N extends S> List<N> getNodesOfClassWithStatus(final Class<N> resultClass, final boolean statusFilter) {
     ...
     * @return the list of nodes matching the qualifier criteria
     */
    public static <S extends AcceleratorNode, N extends S> List<N> getNodesWithQualifier(final List<S> sourceNodes, final TypeQualifier qualifier) {
     ...
     */
    public static <T extends AcceleratorNode> List<T> filterNodesByStatus(final List<T> nodes, final boolean statusFilter) {
        return getNodesWithQualifier(nodes, QualifierFactory.getStatusQualifier(statusFilter));
    }

    /**
     * Shallow fetch of nodes whose type is given by the string. Only nodes that
     * are immediate children of this sequence are filtered. A node will be
     * considered as a match if the specified type matches either the node
     * directly or that of one of the node's superclasses. For example, a
     * quadrupole would match both the "Q" type and the "magnet" type.
     *
     ...
     */
    public <T extends AcceleratorNode> List<T> getNodesOfType(final String strTypeId) {
        return this.<T>getNodesWithQualifier(new KindQualifier(strTypeId));
    }
     ...
     * @return a list of this sequence's nodes which match the specified type
     */
    public <T extends AcceleratorNode> List<T> getNodesOfType(final String strTypeId, final boolean statusFilter) {
        return this.<T>getNodesWithQualifier(new AndTypeQualifier().and(strTypeId).and(QualifierFactory.getStatusQualifier(statusFilter)));
    }

    /**
     * Shallow fetch of nodes that are matched by the qualifier. Only nodes that
     * are immediate children of this sequence are filtered.
     *
     * @param qualifier the qualifier used to filter nodes
     * @return a list of this sequence's nodes which match the qualifier
     * criteria
     */
    public <T extends AcceleratorNode> List<T> getNodesWithQualifier(final TypeQualifier qualifier) {
        return appendNodesWithQualifier(new ArrayList<>(), getNodes(), qualifier);
    }

    /**
     * Fetch all nodes whose type is matched through the qualifier and are also
     * contained in this sequence looking deeply through its nested child
     * sequences. This sequence itself is not among the nodes that will be
     * tested against the qualifier.
     *
     * @param strTypeId the type of node for which we are fetching
     * @return the list of all inclusive nodes which match the qualifier
     * criteria
     */
    public <T extends AcceleratorNode> List<T> getAllNodesOfType(final String strTypeId) {
        return this.<T>getAllNodesWithQualifier(new KindQualifier(strTypeId));
    }

    /**
     * Fetch all nodes whose type is matched through the qualifier and are also
     * contained in this sequence or one of its nested child sequences. This
     * sequence itself is not among the nodes that will be tested against the
     * qualifier.
     *
     ...
     * criteria
     */
    public <T extends AcceleratorNode> List<T> getAllNodesWithQualifier(final TypeQualifier qualifier) {
        return appendNodesWithQualifier(new ArrayList<>(), getAllNodes(), qualifier);
    }

    /**
     * Fetch all nodes which are contained in this sequence looking deeply
     * through its nested child sequences. This sequence itself is among the
     * nodes included, hence the "Inclusive" nature of this method.
     *
     ...
     * Fetch all nodes whose type is matched through the qualifier and are also
     * contained in this sequence looking deeply through its nested child
     * sequences. This sequence itself is among the nodes that will be tested
     * against the qualifier, hence the "Inclusive" nature of this method.
     *
     * @param qualifier the qualifier for filtering the nodes
     * @return the list of all inclusive nodes which match the qualifier
     * criteria
     */
    public <T extends AcceleratorNode> List<T> getAllInclusiveNodesWithQualifier(final TypeQualifier qualifier) {
    ...
    /**
     * Fetch all nodes contained in this sequence looking deeply through its
    ...
    /**
     * Return the allowed predecessor sequences of a sequence At most there can
     * be 2 predecessors.
     *
     ...
     *
     * @param sequence the sequence which we are testing if this sequence can
     * precede
     * @return true if this sequence can precede the sequence specified in the
    ...
    public boolean canPrecede(final AcceleratorSeq sequence) {
        ...
        for (int index = 0; index < predecessors.length; index++) {
            if (strId.equals(predecessors[index])) {
                return true;
            }
        }
        return false;
    ...
    /**
     * Determining if the ordered list of sequences forms a closed loop. The
     * determination is based on whether each successive sequence has a
     * predecessor (based on the getPredecessors() method) that precedes it in
     * the list, and the last item in the list is a predecessor of the first
     * item in the list.
     *
    ...
    public static boolean formsRing(final List<AcceleratorSeq> sequences) {
        ...
        for (int index = 0; index < count; index++) {
            ...
            if (!previousSequence.canPrecede(sequence)) {
                return false;
            }
    ...
    /**
     * Given a collection of sequences, order the sequences according to their
     * predecessor attribute.
     *
     * @param sequences a collection of contiguous sequences to order from start
     * to finish
    ...
    public static List<AcceleratorSeq> orderSequences(final Collection<AcceleratorSeq> sequences) throws SequenceOrderingException {
        ...
        if (sequence == null) {
            return Collections.<AcceleratorSeq>emptyList();
        }
    ...
    /**
     * Internal support for the orderSequences() method. Add this sequence to
     * the beginning of the orderedSequences list and remove it from the
     * sequenceMap. Recursively do the same for its predecessor if it has one in
     * the sequenceMap.
     *
     * @param orderedSequences the list of ordered sequences to append the next
     * sequence
     * @param sequenceMap the table of sequences from which to fetch the next
     * sequence
     */
    ...
    /**
     * Get a list of all of this sequence's subsequences including those deeply
     * nested. return a list of all of this sequence's subsequences
     */
    ...
    /**
     * Return a sequence whose id matches the argument and which is an immediate
     * child of this sequence.
     */
    public AcceleratorSeq getSequence(final String strId) {
        for (final AcceleratorSeq sequence : getSequences()) {
            if (sequence.getId().equals(strId)) {
                return sequence;
            }
        }
    ...
    /**
     * Get the position of a node in the sequence, including the possible offset
     * of a local sequence containing the node.
     *
    ...
    /**
     * Convert the sequence position to a position relative to the specified
     * reference node.
     */
     ...
     * reference node.
     *
     * @param position the position of a location relative to the sequence's
     * start
     * @param referenceNode the node relative to which we wish to get the
     * position
     */
    ...
    /**
     * Get the shortest relative postion of one node with respect to a reference
     * node. This is really useful for ring sequences.
     *
     * @param node the node whose relative position is sought
     * @param referenceNode the reference node relative to which the node's
     * position is calculated
     * @return the distance (positive or negative) of the node with respect to
     * the reference node whose magnitude is shortest
     */
    ...
    /**
     * Identify whether the sequence is within a linear section. This helps us
     * to determine whether it is meaningful to identify one node as being
     * downstream from another. At this time we will simply return true, but
     * when the ring is introduced into the optics, we need to distinguish the
     * ring from the linear sections.
     */
    ...
    /**
     * Sort nodes in place by position where the position used is relative to
     * the this sequence.
     *
    ...
    /**
     * Sort nodes in place by proximity to a reference node. Used to get nodes
     * ranked by nearness to the reference node.
     *
     * @param nodes the list of nodes to sort
    ...
    /**
     * Sort nodes in place by position relative to the reference node (ranked
    ...
    /**
     * Get all sequences in the specified sequence branch, searching down all of
     * its branches
     *
    ...
    /**
     * Get all nodes in the specified sequence searching deeply through this
     * sequence's child sequences.
     *
