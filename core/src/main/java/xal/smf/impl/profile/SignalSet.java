 ...
 * scanner (ergo the identifier <code>SignalSet</code>. Note that these guys are
 ...
 * </p>
 * <p>
 * This class is essentially an active data structure which populates itself
 * with the measurement data from the diagnostic hardware.
 * </p>
 * <p>
 ...
 * of type <code>{@link WireScanner.Signal}</code>.
 * </p>
 * <p>
 * The channel handles needed to connect the <code>Signal</code> attributes to
 * the wire harp device are specified in the derived classes by annotating it
 * with the <code>{@link WireScanner.ASignal}</code> annotation.
 * </p>
 *
 * <p>
...
public abstract class SignalSet implements DataListener {
 ...
 /*
    ...
    /**
     * <p>
     * Returns the set of field descriptors for (field, channel) pairs used by
     * the given<code>SignalSet</code>-derived class. The given signal set must
     * be annotated with the annotation <code>ASignal.ASet</code> for the
     * operation to succeed.
     * </p>
     * <p>
     * Only the descriptors for which the <code>SignalSet</code> annotation has
     * values will be returned. This, of course, includes fields for which
     * <code>SignalSet</code> has default values.
     * </p>
     *
     * @param clsData the <code>SignalSet</code>-derived child class with
     * ASignal.ASet annotations
     *
     * @return set of field descriptors used by the given class which are
     * provided with non-empty values in the <code>SignalSet</code> annotation.
     *
     * @throws ScadaAnnotationException the <code>ASignal.ASet</code> annotation
     * was missing
     *
     * @author Christopher K. Allen
     * @since Sep 27, 2011
...
//                    throw new ScadaAnnotationException("Missing field from annotation " + sig.getClass().getName(), e);
     ...
     * acquisition channels. Since this is an active data structure these
     * channels are used internally to populate the data fields, which is
    ...
    /**
     * Returns the signal object of this set corresponding to the give profile
     * angle.
     *
...
//    
    ...
    /**
     * Populate the fields of this data set by acquiring the current values of
     * the signal process variables of the given device.
     *
    ...
    /**
     * Averages all the signals in the signal set using the given averaging
     * factor. See <code>Signal{@link Signal#average(Signal, double)}</code> for
    ...
    /**
     * Load the contents of this data set from the persistent store behind the
...
//        dia.update(daptSgnls);
    ...
    /**
     * Write out the contents of this measurement data set to the given data
     * store.
     *
     ...
     */
    /**
     * Create a new, uninitialized <code>SignalSet</code> object initialize with
     * values fetched from the given device.
     *
...
//            this.dia = new Signal(ProfileDevice.ANGLE.DIA);
    ...
    /**
     * Create a new <code>SignalSet</code> object initializing it with values
     ...
     * @throws ConnectionException unable to connect to a parameter read back
     * channel
    ...
    /**
     * Creates a new instance of <code>SignalSet</code> and initialize the
     * signal data from the given data source.
     *
     * @param daSrc data source using the <code>DataAdaptor</code> interface
     *
     * @throws IllegalAccessException if an underlying <code>Signal</code> field
     * is inaccessible.
