...
/**
 * Implementation of drift-gap-drift simulation of fieldmaps. Uses numeric
 * integrator for TTF function
 *
...
public class DriftGapDrift extends ElementSeq {
    ...
    public void initializeFrom(LatticeElement latticeElement) {
        ...
        if (fm.getFieldMapFile().endsWith("spokeFieldMap")) {
            pho = -Math.PI;
        } else if (fm.getFieldMapFile().endsWith("medBetaFieldMap")) {
            pho = Math.PI / 2.;
        } else {
            pho = 0.;
        }
