...
public class ElsProbe extends EnvelopeProbe {
    ...
    private GenericMatrix normalizedEmmitance = new GenericMatrix(3, 1);

    private GenericMatrix envelope0;
    private GenericMatrix normalizedEmmitance0;
    ...
    public void initFromTwiss(Twiss[] twiss) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            normalizedEmmitance.setElem(i, 0, twiss[i].getEmittance() * getBeta() * getGamma());
        }
        envelope0 = envelope.copy();
        normalizedEmmitance0 = normalizedEmmitance.copy();
    }
    ...
    public Twiss[] getTwiss() {
        ...
        for (int i = 0; i < 3; i++) {
            twiss[i] = new Twiss(envelope.getElem(3 * i + 1, 0), envelope.getElem(3 * i, 0), normalizedEmmitance.getElem(i, 0) / (getBeta() * getGamma()));
        }
        twiss[2].setTwiss(twiss[2].getAlpha(), twiss[2].getBeta() * Math.pow(getGamma(), 2), twiss[2].getEmittance());
        return twiss;
    }
    ...
    public void reset() {
        ...
        normalizedEmmitance = normalizedEmmitance0.copy();
    }
}
