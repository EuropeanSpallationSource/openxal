...
/**
 * ElogUtility provides convenience methods for interacting with the Elog
 * database.
 *
 * @author tap
 */
public class ElogUtility {
     ...
     */
    public static final int DEFAULT_MAX_BODY_SIZE = 4000;

    /**
     * Default maximum allowed size of the title size
     */
    public static final int DEFAULT_MAX_TITLE_SIZE = 120;

    /**
     * Controls logbook name
     */
    public static final String CONTROLS_LOGBOOK = "Controls";

    /**
     * Operations logbook name
     */
    public static final String OPERATIONS_LOGBOOK = "Operations";

    /**
     * connection dictionary
     */
    protected final ConnectionDictionary connectionDictionary;

    /**
     * database adaptor
     */
    protected final DatabaseAdaptor databaseAdaptor;

    /**
     * Map of binary types keyed by extension
     */
    protected Map< String, BinaryType> binaryTypes;

    /**
     * ElogUtility constructor Uses the default database adaptor.
     *
     * @param dictionary The connection dictionary.
     * @throws DatabaseException if an exception occurs while connecting to the
     * database
     */
    public ElogUtility(final ConnectionDictionary dictionary) throws DatabaseException {
        ...
        databaseAdaptor = adaptor != null ? adaptor : DatabaseAdaptor.getInstance();
    }

    /**
     * Get the default elog utility which uses the default connection dictionary
     * and the default database adaptor.
     *
     * @return The elog utility if the default connection dictionary exists and
     * null if the connection dictionary cannot be found
     * @throws DatabaseException if an exception occurs while connecting to the
     * database
     */
    public static ElogUtility defaultUtility() throws DatabaseException {
        final ConnectionDictionary dictionary = newConnectionDictionary();
        return (dictionary != null) ? new ElogUtility(dictionary) : null;
    }

    /**
     * generate a new connection dictionary appropriate for publishing new
     * logbook entries
     */
    public static ConnectionDictionary newConnectionDictionary() {
    ...
    private Connection newConnection() throws DatabaseException {
        try {
            ...
            throw new DatabaseException("ElogUtility exception at database connection.", databaseAdaptor, exception);
        }
    }
     ...
     * @throws DatabaseException if a database exception occurs while posting an
     * entry
     */
    ...
    /**
     * Post an entry to a logbook. This method is private so that a user who is
     * connected can post an entry only under their own account.
     *
     ...
     * @param badgeNumber The badge number of the user posting the entry
     * @param logbook The name of the logbook to which we are posting the entry
     * @param categories An array of category IDs to associate with the entry
     * (e.g. "OP1", "OP2", etc.).
     * @param title The title of the entry to post
     * @param content The content of the entry to post
     * @throws DatabaseException if a database exception occurs while posting an
    ...
    private void postEntry(final Connection connection, final String badgeNumber, final String logbook, final String[] categories, final String title, final String content) throws DatabaseException {
        try {
            ...
            final Array categoryArray = databaseAdaptor.getArray("LOGBOOK.LOGBOOK_CAT_TAB_TYP", connection, categories);
            ...
            throw new DatabaseException("Exception while posting entry to Elog.", databaseAdaptor, exception);
        }
    }
     ...
     * @param title The title of the entry to post
     * @param content The content of the entry to post
     * @param dataName The name of the associated image or attachment data.
     * @param formatName The format of the image or attachment data to associate
     * with this entry (e.g. "jpg", "pdf", "txt").
     ...
     * with this entry (e.g. "jpg", "pdf", "txt").
     * @param data The image or attachment data to associate with the entry.
     * @throws gov.sns.tools.database.DatabaseException if a database exception
     * occurs while posting an entry
     */
    private void postEntry(final Connection connection, final String badgeNumber, final String logbook, final String title, final String content, final String dataName, final String formatName, final byte[] data) throws DatabaseException {
        ...
        if (!binaryTypes.containsKey(formatName)) {
            throw new IllegalArgumentException("\"" + formatName + "\" is not a valid binary data type");
        }

        final BinaryType binaryType = binaryTypes.get(formatName);
     ...
     * @throws gov.sns.tools.database.DatabaseException if a database exception
     * occurs while fetching the badge number
     */
    private String getBadgeNumber(final Connection connection) {
        try {
            final String userID = connectionDictionary.getUser().toUpperCase();
            ...
            throw new DatabaseException("Exception while fetching the badge number.", databaseAdaptor, exception);
        }
    }
    ...
    private void fetchAttachmentTypes(final Connection connection) throws DatabaseException {
        try {
            ...
            throw new DatabaseException("Exception while fetching the attachment types.", databaseAdaptor, exception);
        }
    }
}
