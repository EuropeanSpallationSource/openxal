...
/**
 * <p>
 * Class for processing profile data contained in <code>ProfileData</code>
 * objects.
 * </p>
 * <p>
 * The objective of this class is to interpolate any missing data (represented
 * by a value <code>Double.NaN</code>), remove the noise baseline, decouple the
 * cross-talk between the signals, then gate the noise in the signal. Thus, it
 * is expected that the data is processed by the following sequence of method
 * calls:
 * <br>
 ...
 * <br>
 * <br>
 * Once this sequence is completed the processed signals will be approximately
 * zero base-lined, decoupled, and normalized to have unity first integral.
 * </p>
 * <p>
 * One may apply the above sequence of calls using default values for all the
 * arguments with the single call to
 * <code>ProfileDataProcessor.{@link #processData()}
 * </p>
 * <p>
 * There are three numeric tuning parameters that must be provided in the
 * processing.
 * <br>
 * <br>ProfileDataProcessor.{@link #removeBaselineNoise(double dblSigThreshold)}
 * <br><var>dblSigThreshold</var> = amplitude of indicator function deciding
 * signal presence
 * <br>This value is in the interval [0,1]. Larger will use more signal samples
 * to determine noise properties, but may also include actual signals. Smaller
 * values are more likely to exclude signals in the noise calculations, but will
 * use few samples to characterize the noise.
 * <br>
 * <br>ProfileDataProcessor.{@link #decoupleSignals(int cntSmpMax, double dblFracSigMin)}
 * <br><var>cntSmpMax</var> = number of samples to average when computing
 * coefficients
 * <br>This value must be an integer greater than 1. This value determined the
 * number of samples around the maximum value used when computing coupling
 * coefficients. Small numbers will use samples with the most integrity. Larger
 * numbers will include samples more affected by noise, but may provide a better
 * statistical approximation if the data set contains suspect data.
 * <br><var>dblFracSigMin</var> fractional signal strength necessary for
 * inclusion in coupling calculation
 * <br>This value ranges in the interval [0,1]. Large values ensure that only
 * the best samples are used in computing the coupling coefficients, however, it
 * reduces the available sample size. Small values increase the sample size but
 ...
 * value of 1 would cause a filter output of zero for all inputs. Although sign
 * is not enforced per se, we are expecting a positive signal. Thus, a value for
 * <var>dblFracSigMax</var> in the interval (0,1) is appropriate. Then if the
 * projection data were strictly non-negative, a value 0 would indicate no
...
public class ProfileDataProcessor {
    ...
    /**
     * Minimum fractional signal amplitude necessary to include sample for
     * coupling coefficient calc.
     */
     ...
     */
    /**
     * Create a new <code>ProfileDataProcessor</code> object attached to the
     * given profile data object.
     *
     ...
     *
     * @throws IllegalArgumentException this should not occur (serious internal
     * error)
     *
    ...
    /**
     * <p>
     * Perform the default processing on the associated projection data. The
     * following operations are performed:
     * <br>
     * <br>&nbsp;&nbsp; {@link ProfileDataProcessor#interpolateMissingData()}
     * <br>&nbsp;&nbsp; {@link ProfileDataProcessor#removeBaselineNoise(double)}
     * <br>&nbsp;&nbsp;
     * {@link ProfileDataProcessor#decoupleSignals(int, double)}
     * <br>&nbsp;&nbsp; {@link ProfileDataProcessor#thresholdFilter(double)}
     * <br>
     * <br>
     * Default values for the arguments are taken from the global constants of
     * this class.
     * </p>
     *
    ...
    /**
     * Interpolate between missing data points. Missing data is indicated with a
     * value <code>Double.NaN</code>. If we encounter more than two contiguous
     * bad data points the operation punts by throwing an exception.
     *
     * @throws DataProcessingException data set exceeds maximum bad data points
     */
     ...
     * </p>
     *
     * @param dblSigThreshold amplitude of indicator function deciding signal
     * presence
     *
    ...
    /**
     * <p>
     * Decouple the project data signals. That is, remove the cross-talk signals
     * from each projection data set.
     * </p>
     * <p>
     ...
     * <strong>NOTE:</strong>
     * <p>
     * It is best to use this method after the baseline noise have been removed
     * from the profile data.
     * </p>
     *
     * @param cntSmpMax maximum number of samples to average when computing
     * coefficients
     * @param dblFracSigMin minimum fractional signal strength for use in
     * calculation
     */
     ...
     * </p>
     * <p>
     * Apply a threshold filter to each projection data signal using the given
     * fractional amplitude. Specifically, the projection data signal strength
     * must be strictly greater than <var>dblFracSigMax</var>
     * of the maximum signal strength to achieve a nonzero value. If the value
     * <var>dblFracSigMax</var> is chosen properly the filter will perform as a
     * noise gate where we assume a positive signal in the projection data.
     * </p>
     * <p>
     * NOTE:
     * <br>- This is a zero-order filter (no memory), but it is highly
     * nonlinear. Thus, you need to be careful when using this method,
     * especially for computations with the resulting data.
     * <br>- The argument is the fraction of the maximum signal strength where
     * the threshold is activated. Thus, this value is in the interval (-1,1)
     * where a value of 1 would cause a filter output of zero for all inputs.
     * <br>- Although sign is not enforced per se, we are expecting a positive
     * signal. Thus, a value for <var>dblFracSigMax</var> in the interval (0,1)
     * is appropriate. Then if the projection data were strictly non-negative, a
     ...
     */
    /**
     * Write out contents both original data and processed data to a string for
     * inspection. The returned string is a formatted table of values as
     * produced by <code>DigitalFunctionUtility#buildValueTable()</code>.
     *
    ...
    /**
     * Fills in bad data points with a linear interpolation using its neighbors.
     * If two contiguous bad points are found (two <code>NaN</code> values) then
     * an exception is thrown.
     *
     * @param arrFunc discrete function containing bad data points
     *
     * @throws DataProcessingException encountered two contiguous bad data
     * points
     */
     ...
     * @param dblFracMaxSig signal strength must be this fraction of max signal
     * to be used in average
     */
     ...
     * <code>ProfileDataProcessor{@link #computeXTalk(int)}</code> but before
     * the gain and cross-talk coefficients are separated with the call to
     ...
     * <p>
     * This method must be called after
     * <code>ProfileDataProcessor{@link #computeXTalk(int)}</code> and
