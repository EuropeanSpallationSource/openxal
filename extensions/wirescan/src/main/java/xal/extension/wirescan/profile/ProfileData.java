 ...
 * angles are enumerated with the Java enumeration <code>Angle</code>.
 * </p>
 * <p>
 * Also contained in this data set are the axes positions for each projection
 * sample. Specifically, each projection has an associated vector of axis
 * positions where the sample was taken. Moreover, a time stamp for the data and
 * the device identifier for the source of the data is stored.
 * </p>
 * <p>
 * <br>NOTES:
 * <br> - Currently there are no initializing constructors, or public
 * constructors for that matter. There is a
 * <code>create(String, Date, int)</code> static method for directly
 ...
 * if you wish to create a <code>ProfileData</code> object directly, you must
 * call the factory method <code>create(String, Data, int)</code> then pack the
...
public class ProfileData {
    ...
    public enum DeviceType {
        ...
        /**
         * Return the device type which is inferred from the given device
         * identifier string. Specifically, we assume that the device identifier
         * string <code>strDevId</code> is prefixed by the (two-character)
         * device type string. If the first two characters of
         * <code>strDevId</code> are not a recognized device type identifier
         * string, the device type <code>UNKNOWN</code> is returned.
         *
         * @param strDevId device identifier string
         *
         * @return  <code>DeviceType</code> object corresponding to given device
         * id
         */
    ...
    /**
     * <p>
     * Create a new, uninitialized instance of <code>ProfileData</code>,
     ...
     */
    /**
     * Resets the data. Space for the given array size is allocated. Any
    ...
    /**
     * Sets the device identifier string for this data. Also sets the device
     * type as determined from the string identifier.
     *
    ...
    /**
     * Clears the missing data flag. For use by data processing objects which
     * are capable of "fixing" missing data points.
     */
    ...
    /**
     * Set the entire actuator position vector. This is a copy operation, not a
     * set by reference. Note further that the given array must have length
     * equal to the storage specified by
     * <code>ProfileData#allocStorage(int)</code>.
     *
    ...
    /**
     * Set the entire actuator position vector. Note that the given list must
     * have length equal to the storage specified by
     * <code>ProfileData#allocStorage(int)</code>.
     *
    ...
    /**
     * Set the entire axis position vector for the given viewing angle. This is
     * a copy operation, not a set by reference. Note further that the given
     * array must have length equal to the storage specified by
     * <code>ProfileData#allocStorage(int)</code>.
     *
     * @param view viewing angle
    ...
    /**
     * Set the entire projection data array. This is a copy operation, not a set
     * by reference. Note further that the given array must have length equal to
     * the storage specified by <code>ProfileData#allocStorage(int)</code>.
     *
     * @param view projection view angle
     * @param arrVals array of projection data
     *
     * @throws ArrayIndexOutOfBoundsException data size not equal storage
     * allocated
     */
    ...
    /**
     * Set the entire projection data array. Note that the given list must have
     * length equal to the storage specified by
     * <code>ProfileData#allocStorage(int)</code>.
     *
     * @param view projection view angle
    ...
    /**
     * Return the missing data flag. If <code>true</code> then this data set
     * contains missing points which are represented with the value
     * <code>Double.NaN</code>. Otherwise, the data set is complete.
     *
    ...
    /**
     * Return the Process Variable Logger (PVLogger) record identifier for the
     * machine configuration when this data was taken
     *
     ...
     *
     * <p>
     * NOTE: For the sake of speed, there is no error checking here. Any bounds
     * overruns bubble up from here.
     * </p>
     *
     ...
     */
    /**
     * Check to see that given object is either this object, or contains exactly
     * the same data.
     *
     * @param objData object under comparison
     *
     * @return true if <var>objData</var> is <code>this</code> or contains the
     * same data
     *
     ...
     */
    /**
     * Check that the number of elements in the variable argument array is equal
     * to the number of projection views.
     *
     * @param arrArgs array of argument objects
     *
     * @throws IllegalArgumentException number of arguments not equal to number
     * of views
     *
     ...
     *
     * @return true if <var>arr1</var> = <var>arr2</var> as vectors, false
     * otherwise
     */
