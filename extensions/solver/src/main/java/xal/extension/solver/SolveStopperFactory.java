...
public class SolveStopperFactory {
    ...
    /**
     * Stop the solver after the solver reaches max evaluations (or algorithm
     * executions to avoid a possible hang).
     *
     * @param maxEvaluations The maximum evaluations or algorithm executions to
     * run the solver.
     * @return The stopper implementation.
     */
     ...
     *
     * @param maxSeconds The maximum number of seconds before getting the
     * stopper.
     * @return A stopper.
     */
    ...
    /**
     * Get a stopper that stops after the minimum satisfaction is achieved. This
     * stopper should not be used alone since it may never stop if the
     * satisfaction is unreachable.
     *
     * @param satisfactionTarget The satisfaction that must be reached by all
     * objectives before stopping.
     * @return A stopper.
     */
    ...
    public static Stopper maxEvaluationsSatisfactionStopper(final int maxEvaluations, final double satisfactionTarget) {
        return new Stopper() {
            ...
            public boolean shouldStop(final Solver solver) {
                final ScoreBoard scoreboard = solver.getScoreBoard();
                if (scoreboard.getEvaluations() >= maxEvaluations || scoreboard.getAlgorithmExecutions() >= maxEvaluations) {
                    return true;
                }
    ...
    /**
     * Get a stopper that runs between a minimum and maximum time and has a
     * minimum satisfaction that all objectives must reach in order to stop
     * short of the maximum time.
     *
    ...
    /**
     * Get a stopper that stops after the specified number of repeat solutions
     * is found.
     *
     * @param minRepeatSolutions the number of repeat solutions to find before
     * stopping
     */
    ...
    /**
     * Compound stopper which stops the solver if either stopper1 or stopper2
     * would stop it.
     *
    ...
    /**
     * Compound stopper which stops the solver if both stopper1 and stopper2
     * would stop it.
     *
     * @param stopper1 The first stopper to check
     * @param stopper2 The second stopper to check
