 ...
 * <code>java.lang.String</code>. <code>WheelswitchFormatter</code> also stores
 * the value, its bounds (minimum and maximum) and also the unit displayed by
 * the <code>Wheelswitch</code>.
 *
...
public class WheelswitchFormatter {
    ...
    /**
     * Checks the number format string. The format string should only consist of
     * characters '#' reperesenting a single number digit, '+' representing a
     * sign digit, '.' representing the decimal symbol digit and 'E'
     ...
     * numerical digits would result in a correct double expression acceptible
     * by <code>Double.parseDouble()</code>.
     *
     * @param format
     *
     ...
     * strings and wheelswitch type format strings. The transform can be used in
     * either direction. The method does not check for validity of the supplied
     * parameter string and may return bogus results if invalid format strings
     * are passed to it.
     *
