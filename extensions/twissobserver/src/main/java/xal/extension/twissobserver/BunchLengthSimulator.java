 ...
 * In order to do computation with space charge it is necessary to have
 * longitudinal information since dynamics in all three phase planes are coupled
 * by the electro- magnetic fields. This tools provides a simulated longitudinal
 * bunch size to fill in the missing information.
 *
 * @author Christopher K. Allen
 * @since Sep 6, 2012
 */
public class BunchLengthSimulator {
     ...
     */
    /**
     * Creates a new <code>BunchLengthSimulator</code> object using the given
     ...
     *
     * @throws ModelException an error occurred when instantiating the machine
     * model
     */
    ...
    /**
     * <p>
     * Generates all the bunch lengths at the given device locations for the
     * given measurement data. The default initial beam state is used to start
     * the beam simulation at the entrance of the accelerator sequence.
     * </p>
     *
     * @param arrMsmts the measurement data to be packed with longitudinal dummy
     * data
     ...
     * Generates all the bunch lengths at the given device locations for the
     * given measurement data. The default initial beam state is used to start
     * the beam simulation at the entrance of the accelerator sequence. The beam
     * sizes are multiplied by the scaling factor provided; this is used to
     * convert units since the online model units are all MKS. For example, use
     * 1000.0 to convert from meters (XAL) to millimeters.
     * </p>
     *
     * @param arrMsmts the measurement data to be packed with longitudinal dummy
     * data
     ...
     * <p>
     * Generates all the bunch lengths at the given device locations for the
     * given measurement data. The simulation is performed with the given bunch
     * charge and the given initial beam state at the start of the accelerator
     * sequence. The beam sizes are multiplied by the scaling factor provided;
     * this is used to convert units since the online model units are all MKS.
     * For example, use 1000.0 to convert from meters (XAL) to millimeters.
     * </p>
     *
     * @param arrMsmts the measurement data to be packed with longitudinal dummy
     * data
     ...
     * or the default value is used if <code>null</code>
     *
     * @throws ModelException general error during model synchronization or
     * simulation
     *
     ...
     */
    /**
     * Runs the actual online model simulation. Returns the trajectory object
     ...
     *
     * @throws ModelException the simulation trajectory was not of type
