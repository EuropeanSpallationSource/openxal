...
/**
 * <p>
 * Base class for the problem of reconstructing Courant-Snyder parameters from
 * beam profile data at multiple locations along a beamline. This class provide
 * common tools and attributes that child classes can share, as well as the
 * ability to compute the zero-current solution.
 * </p>
 * <p>
 * Subclasses compute the covariance matrix <b>&sigma;</b> from the given data
 ...
 * which, in the zero current case, is independent of the initial state of the
 * beam. The observation matrix relates the initial state (described by the
 * second moments) to the observed quantities, in this case the measurement data
 * of RMS beam sizes. For each phase plane: In the case were there are more data
 * points than there are variables (i.e., greater than 3) then the returned
 * solution is the least-squares solution; specifically, the set of moments that
 * hits the most data. In the case where there are less data points than
 * variables (i.e., less than 3) then the returned solution is the minimum norm
 * (or "least energy") solution; specifically, the solution that has the
 * smallest Lebesgue 2-norm. If we have 3 data points for the phase plane then
...
public abstract class CourantSnyderEstimator {
     ...
     * at the given device location from the data provided and for the given
     * beam current.
     * </p>
     *
     ...
     *
     * @throws Exception general error occurred during the reconstruction
     * computations
     *
     * @author Christopher K. Allen
     * @since Apr 2, 2013
 ...
 /*
     ...
     */
    /**
     * Creates a new <code>TwissObserverBase</code> object which uses the given
     * transfer matrix generator.
     *
     * @param genTransMatrix a pre-configured transfer matrix engine used
     * internally.
     *
     * @author Christopher K. Allen
     * @since Sep 4, 2012
    ...
    /**
     * <p>
     * Creates a new <code>TwissObserverBase</code> object using the given
     * maximum iteration count, maximum convergence error, and the given
     * transfer matrix generator.This constructor must be used if Twiss
     * parameters are to be computed in the presence of space charge (see
     * <code>{@link #computeCovarianceFiniteCurrent(String, double, ArrayList)}</code>).</p>
     *
     ...
     *
     * @param bolDebug  <code>true</code> to enable debugging output or
    ...
    /**
     * <p>
     * Returns the current solution covariance matrix. This matrix is useful if
     * an iterative solution algorithm was used (e.g., for the finite space
     * charge) and it did not converge. The returned value is then the last
     * value of the solution before the algorithm was terminated.
     * </p>
     * <h3>NOTE:</h3>
     * <p>
     * &middot; It is the responsibility of the subclass to keep this value up
     * to date.
     * </p>
     *
     * @return covariance matrix being the last computed solution value in an
     * iterative algorithm
     *
     * @author Christopher K. Allen
     * @since Nov 21, 2012
    ...
    /**
     * <p>
     * Returns the residual error of the last solution, i.e., the solution
     * returned by <code>{@link #getReconstruction()}</code>. The numerical
     * value is the Frobenius norm of the solution residual matrix.
     * <h3>NOTE:</h3>
     * <p>
     * &middot; It is the responsibility of the subclass to keep this value up
     * to date.
     * </p>
     *
     * @return ||<b>&Omega;&sigma;</b><sup>*</sup> -
     * <b>X</b>||<sub><i>F</i></sub>
    ...
    /**
     * <p>
     * Returns the convergence error for the last solution iteration, i.e., the
     * distance between the solution returned by
     * <code>{@link #getReconstruction()}</code> and the solution previous. The
     * numerical value is the Frobenius norm of the distance between iterates.
     * <h3>NOTE:</h3>
     * <p>
     * &middot; It is the responsibility of the subclass to keep this value up
     * to date.
     * </p>
     *
     * @return || <b>&sigma;</b><sub><i>i</i>+1</sub> -
     * <b>&sigma;</b><sub><i>i</i></sub> ||<sub><i>F</i></sub>
     * &nbsp where <b>&sigma;</b><sup>*</sup> is the solution
     *
     * @author Christopher K. Allen
     * @since Nov 21, 2012
     ...
     */
    /**
     * Returns the recursion function for a zero-current beam at the given
     * device location and for the given profile measurement data. This method
     * actually returns the results of
     ...
     *
     * @param strRecDevId device ID where the Courant-Snyder parameters are
     * being reconstructed
     * @param arrData profile measurement data along beamline
     *
     * @return covariance matrix containing the value of the recursion function
     * generated from
     * <var>matSig0</var> and <var>dblChrg</var>
     *
     * @throws ModelException a general error occurred while computing the
     * transfer matricies
     *
     * @author Christopher K. Allen
     * @since Apr 18, 2013
    ...
    /**
     * Returns the recursion function for a finite current beam with the given
     * beam charge for the given beam state at the sequence start. This method
     * actually returns the results of
     * <code>{@link #computeReconSubFunction(PHASEPLANE, String, ArrayList)}</code>
     * simultaneously for all three phase planes as a covariance matrix. In this
     * method, the transfer matrices are explicitly computed for the given
     * initial covariance matrix and the given beam current. The returned matrix
     * is block diagonal (and symmetric) containing the moments in the usual
     * locations.
     *
    ...
    /**
     * <p>
     * Compute the observation matrix which yields the vector of beam moments at
     * the given beamline element from the set of beam size measurements
     * provided in the argument. The matrix is returned as a
     * <code>eMatrix</code> object so that matrices for all three phase planes
     * may be returned. Currently coupling between phase planes is not consided.
     * Thus, the returned matrix is block-diagonal having three
     * 3<i>n</i>&times;<i>9</i>
     * matrices on the diagonal (where <i>n</i> is the number of data
     * locations), one for each phase plane, horizontal, vertical, diagonal,
     * respectively.
     * </p>
     * <h3>NOTES:</h3>
     * <p>
     * &middot; The transfer matrix generator object is accessed to retrieve
     * whatever transfer matrices it contains when this method is called.  <b>BE
     * CAREFUL</b> to generate the appropriate transfer matrices before this
     * method is called.
     * </p>
     *
     ...
     *
     * @return a <code>PhaseMatrix</code> containing the observation matrices
     * for all 3 phase planes
     *
    ...
    /**
     * <p>
     * Computes the <b>&sigma;</b> vector from the observer matrix and the given
     ...
     * <br>
     * <br>
     * &nbsp; &nbsp; <b>&sigma;</b> = ( &lt;<i>x</i><sup>2</sup>&gt;,
     * &lt;<i>xx'</i>&gt;, &lt;<i>x'</i><sup>2</sup>&gt; )<sup>T</sup>
     * <br>
     * <br>
     * where the moments are taken from the horizontal plane. The vertical plane
     * computation returns an analogous set. The three phase planes are treated
     * independently and solutions for each plane returned separately.
     * </p>
     * <p>
     * In the case were there are more data points than there are variables
     * (i.e., greater than 3) then the returned solution is the least-squares
     * solution; specifically, the set of moments that hits the most data. In
     * the case where there are less data points than variables (i.e., less than
     * 3) then the returned solution is the minimum norm (or "least energy")
     * solution; specifically, the solution that has the smallest Lebesgue
     * 2-norm. If we have three data points then the unique solution is
     * returned.
     * </p>
     * <h3>NOTES:</h3>
     * <p>
     * &middot;  <em>IMPORTANT</em>: The moments are computed using the transfer
     * matrices currently stored in the internal transfer matrix generator
     * object. Thus, whatever generation was used prior to this call is used to
     * compute the recursion sub-function.
     * <br>
     * &middot; If the reconstruction device has non-zero length the location of
     * the moments corresponds to the exit of the device.
     * </p>
     *
     ...
     *
     * @return Resulting <b>&sigma;</b> vector of second moments at the given
     * device location
     */
    ...
    /**
     * <p>
     * Computes the observation matrix from the given data and accelerator node
     * ID of the reconstruction location.
     * </p>
     * <h3>NOTES:</h3>
     * <p>
     * &middot; The transfer matrix generator object is accessed to retrieve
     ...
     * @see TransferMatrixGenerator#generateWithSpaceCharge(String, double,
     * CovarianceMatrix)
     *
     * @throws IllegalArgumentException unknown phase plane or unknown device
     * @throws IllegalStateException transfer matrices have not been generated
    ...
    /**
     * Computes and returns the Lebesgue 2-norm of the error residual for the
     * given solution and problem data.
     *
     * @param matSigSoln solution matrix of second-order moments
     * @param strRecDevId device ID where the reconstruction is performed
     ...
     * matrix and the last solution matrix. This method should be called under
     * the assumption that an iterative solution technique is being employed and
     * we are monitoring a Cauchy sequence to check for convergence.
     *
     * @param matSig1 current solution iterate
     * <b>&sigma;</b><sub><i>i</i>+1</sub>
     * @param matSig0 previous solution iterate
     * <b>&sigma;</b><sub><i>i</i></sub>
     *
     * @return the (Frobenius) distance between the current iterate and the
     * previous one || <b>&sigma;</b><sub><i>i</i>+1</sub> -
     * <b>&sigma;</b><sub><i>i</i></sub> ||<sub><i>F</i></sub>
     *
     * @author Christopher K. Allen
     * @since Apr 5, 2013
    ...
    /**
     * Takes the given measurement data and packages it into an appropriate
     * vector for the given phase plane. Also, converts the measurement RMS
     * values (sigmas) into the second-order moment values.
     *
