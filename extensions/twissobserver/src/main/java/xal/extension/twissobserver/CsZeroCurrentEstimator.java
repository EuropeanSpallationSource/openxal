 ...
 * data measurements along a beamline. This class is a simple extension of the
 * <code>{@link TwissObjserverBase}</code> class providing only public access to
 * the tools for computing the recursion function in the case of zero beam
 * current.
 *
 * @author Christopher K. Allen
 * @since Apr 18, 2013
...
public class CsZeroCurrentEstimator extends CourantSnyderEstimator {
     ...
     * Creates a new instance of <code>ZeroCurrentSolution</code>.
     *
     * @param bolDebug output debug information to the console if
     * <code>true</code>
     ...
     * the measurement data cannot show coupling between phase planes, the
     * covariance matrix is block diagonal where the 2&times;2 blocks correspond
     * to each independent phase plane (see
     * <code>{@link CorrelationMatrix}</code>). If the measurement data is zero
     * for any phase plane, then the diagonal block corresponding to that phase
     * plane is also zero.
     * </p>
     * <p>
     ...
     * measurement data of RMS beam sizes. For each phase plane: In the case
     * were there are more data points than there are variables (i.e., greater
     * than 3) then the returned solution is the least-squares solution;
     * specifically, the set of moments that hits the most data. In the case
     * where there are less data points than variables (i.e., less than 3) then
     * the returned solution is the minimum norm (or "least energy") solution;
     * specifically, the solution that has the smallest Lebesgue 2-norm. If we
     * have 3 data points for the phase plane then the unique solution is
     * returned.
     * </p>
     * <h3>NOTE</h3>
     * <p>
     ...
     */
    /**
     * Fulfills the requirement of the base class abstract function. The beam
     * charge parameter is ignored. This method is simply a proxy to
