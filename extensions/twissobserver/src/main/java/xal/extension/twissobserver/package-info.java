 ...
 * parameters
 * <strong>&sigma;</strong> at a hardware location along the beamline from the
 * RMS beam sizes downstream of that that location. Rather than running a model
 * of the beamline and adjusting the initial conditions of the beam (i.e., the
 ...
 * <strong>F</strong> from the Courant-Snyder parameter domain to itself. This
 * is direct approach to the problem rather than the "weak" solution found by
 * the fitting technique where the Courant-Snyder parameters minimizing a
 * functional. Here the map is iterated to the fixed point of
 * <strong>F</strong>, the fixed point
 * <strong>&sigma;</strong> = <strong>F&sigma;</strong> being the solution of
 * Courant-Snyder parameters.
 * </p>
 * <p>
 * Let <strong>T</strong> : <em>CS</em> &rarr; <em>D</em> be the map that takes
 * Courant-Snyder parameters to the set of RMS beam sizes along the beamline.
 * The map <strong>T</strong>
 ...
 * <br>
 * <br>
 * where <strong>T</strong><sup>*</sup> is the adjoint of <strong>T</strong> and
 * <strong>d</strong> is the vector of RMS beam sizes (i.e., the data). When no
 * space charge is present
 * <strong>T</strong> is a linear map and this equation may be solved directly.
 * </p>
 * <p>
 * When no space charge is present, indicated by <em>I</em> &gt> 0 where
 * <em>I</em>
 * is the generalized beam current, the map <t> now depends upon the initial
 * Courant-Snyder parameters, that is
 * <strong>T</strong> = <strong>T</strong>(<strong>&sigma;</strong>). For any
 * given beam current <em>I</em> we define the map
 ...
 * <br>
 * <br>
 * The fixed point <strong>&sigma;</strong>(<em>I</em>) of <strong>F</strong> is
 * the set of Courant-Snyder parameters for the beam current <em>I</em>.
 * </p>
 * <p>
 * One particularly simply way of solving this problem is to pick an initial set
 * of Courant-Snyder parameters <strong>&sigma;</strong><sub>0</sub>(<em>I</em>)
 * and start iterating the above equation. If
 * <strong>&sigma;</strong><sub>0</sub>(<em>I</em>) is within the region of
 * contraction for the fixed point of
 * <strong>F</strong> then the iteration converges to the solution. We can
 * improve the convergence properties by forming an outside loop where
 * <em>I</em> is increased from 0 to its target values. That is,
 * <strong>F</strong> is iterated to its fixed point for each increment of beam
 * current, and that fixed point is used to initialize the next iteration.
 * <p>
 * </p>
 * Another more sophisticated technique is a continuation method exploiting the
 * smoothness of
 * <strong>F</strong>(&middot;, &middot;). Starting with
 * <strong>F</strong>[<strong>&sigma;</strong>(<em>I</em>), <em>I</em>] =
 * <strong>&sigma;</strong>(<em>I</em>) we take the total derivative with
 * respect to <em>I</em> yielding
 * <br>
 * <br>
 * &nbsp; &nbsp; &part;<sub>I</sub><strong>&sigma;</strong>(<em>I</em>) =
 * [<strong>Id</strong> -
 ...
 * <br>
 * <br>
 * where <strong>Id</strong> is the identity map. Starting from
 ...
 * <strong>&sigma;</strong>(&middot;) since the above linear extrapolation
 * cannot account for curvature.
 * </p>
 * <p>
 * </p>
 ...
 * <br>
 * <br>
 * &nbsp; &nbsp; <code><strong>CsZeroCurrentEstimator</strong></code> - This
 * class is used for estimating the Courant-Snyder parameters whenever space
 * charge effects are negligible. As described above, in this case it is a
 * direct calculation and very fast. The class should always be used for such a
 * case.
 * <br>
 * <br>
 * &nbsp; &nbsp; <code><strong>CsFixedPointEstimator</strong></code> - This
 * class estimates Courant-Snyder parameters using the fixed point iteration
 * method, with space charge. At current this class performs very well; its
 * convergence properties are good and it is fast.
 * <br>
 * <br>
