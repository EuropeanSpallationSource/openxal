...
/**
 * <p>
 * Class for generating transfer matrices for the XAL accelerator beamlines. For
 * economic reasons the simulation is first run using methods
 * {@link #generateWithoutSpaceCharge()} for the zero-current case or
 * {@link #generateWithSpaceCharge(double, double, CovarianceMatrix)} for the
 * finite beam current case. Once the simulation is completed transfer matrices
 * between any two beamline elements may be retrieved with the method
 * {@link #retrieveTransferMatrix(String, String)}. Note that when space charge
 * is present an initial covariance matrix for the beam must be supplied. The
 * matrix describes the beam sizes at the entrance to the beamline.
 * </p>
 *
 * @author Eric Dai
...
public class TransferMatrixGenerator {
    ...
    public enum SYNC {
        ...
        /**
         * Synchronize to the current live values, except for the RF parameter
         * which are design
         */
         ...
         */
        /**
         * Returns the value of the synchronization mode represented by this
         * constant, as used by the <code>{@link Scenario}</code> class.
         *
        ...
        /**
         * Enumeration constant constructor - initialize the synchronization
         * mode value.
         *
         * @param strSyncMode synchronization mode string used by
         * <code>Scenario</code>
         */
    ...
    /**
     * Indicates whether space charge was included in the transfer matrix
     * calculation
     */
     ...
     * sequence, and synchronizes to the design parameters.
     *
     * @param smfSeq the desired sequence for transfer matrix computation
     *
     * @throws ModelException and error occurred when instantiating the machine
     * model
     */
    ...
    /**
     * Creates a new <code>TransferMatrixGenerator</code> object using the given
     * sequence, initializes the model to the historical machine state
     * identified by the given PV Logger ID.
     *
     * @param smfSeq the desired sequence for transfer matrix computation
     * @param lngPvLogId PV Logger ID of the historical machine snapshot where
     * model parameters are taken
     *
     * @throws ModelException and error occurred when instantiating the machine
     * model
     */
    ...
    /**
     * Creates a new <code>TransferMatrixGenerator</code> object using the given
     * accelerator (XML definition file) accelerator, the given sequence, and
     * the given synchronization mode.
     *
     * @param smfSeq the desired sequence for transfer matrix computation
     * @param enmSyn source of machine parameters
     *
     * @throws ModelException and error occurred when instantiating the machine
     * model or a component
     */
     ...
     * synchronization mode determines where this parameters are taken from
     * (e.g., the live machine, the design parameters, etc.). It is also
     * possible to synchronize the hardware parameters to a historical machine
     * state with a know PV Logger ID (see
     * <code>{@link #setSyncToMachineHistory(long)}</code>).
     *
     * @param enmSync synchronization mode determining parameter source
     *
     * @throws SynchronizationException general error during model
     * synchronization
     *
     * @author Christopher K. Allen
     * @since Jul 19, 2012
     ...
     * snapshot ID. The transfer matrices provided will be from this machine
     * state.
     *
    ...
    /**
     * <p>
     * Generates all the transfer matrices for the zero-current case. The model
     * is run using a transfer map probe which only collects machine data.
     * </p>
     * <h3>NOTE:</h3>
     * <p>
     * &middot; This simulation always starts with the first element in the
     * sequence since the transfer matrices are not dependent upon beam state.
     * </p>
     *
     * @throws ModelException general error during model synchronization or
     * simulation
     *
     * @author Christopher K. Allen
     * @since Jul 26, 2012
    ...
    /**
     * <p>
     * Generates all the transfer matrices including space charge effects. The
     * simulation is performed with the given bunch charge and the initial beam
     * state at the entrance to the associated accelerator sequence.
     * </p>
     * <p>
     ...
     * <h3>NOTE</h3>
     * <p>
     * &middot; Bunch charge (the target parameter) is given by beam current
     * divided by bunch frequency.
     * </p>
     *
     ...
     * </p>
     * <p>
     * There is a self-consistency issue between the space charge effects and
     * the beam size. Thus, the transfer matrices will typically be different
     * for different starting configurations of the beam.
     * </p>
     * <h3>NOTE:</h3>
     * <p>
     ...
     * <code>null</code> for the beginning of the sequence
    ...
    /**
     *
     * Compute and return the transfer matrix from the beginning of the current
     * accelerator sequence to the entrance of the given element. This method is
     * viable only after a call to methods
     * <code>{@link #generateWithoutSpaceCharge()}</code> or
     * <code>{@link #generateWithSpaceCharge(String, double, CovarianceMatrix)}</code>.
     *
     * @param strElemStop string identifier of the terminal element
     *
     * @return beam optics transfer matrix for the beam line up to the given
     * element
     *
     * @throws IllegalStateException The transfer matrices have not been
     * generated yet
     *
    ...
    /**
     * Compute and return the transfer matrix from the entrance of the given
     * starting element to the entrance of the given stop element (within the
     * current accelerator sequence). This method is viable only after a call to
     * methods <code>{@link #generateWithoutSpaceCharge()}</code> or
     * <code>{@link #generateWithSpaceCharge(String, double, CovarianceMatrix)}</code>.
     *
