...
public class Commander {
    ...
    /**
     * Load a custom bundle if one exists. If a custom bundle exists, it will
     * override and extend the properties found in the default bundle. You can
     * use it to customize your toolbar, menubar and menus.
     *
    ...
    /**
     * Load a custom bundle for the document if one exists. If a custom document
     * bundle exists, it will override and extend the properties found in the
     * application bundle. You can use it to customize your toolbar, menubar and
     * menus.
     *
    ...
    /**
     * Append to the menu bar, the menus associated with the specified keys and
     * any preceding and following menus if specified for any of the added
     * menus.
     *
    ...
    /**
     * Append to the menu bar, the menu associated with the specified key and
     * any preceding and following menus if specified for the menu.
     *
     * @param menuBar the menu bar to which to append the menu
    ...
    private void appendMenubarMenuWithKey(final JMenuBar menuBar, final String menuKey) {
        if (getState(menuKey).isIncluded()) {
            ...
            if (precedingMenuKeys != null) {
                appendMenubarMenusWithKeys(menuBar, precedingMenuKeys);
            }
            ...
            if (followingMenuKeys != null) {
                appendMenubarMenusWithKeys(menuBar, followingMenuKeys);
            }
        }
    }

    /**
     * Append to the menu, the menu items associated with the specified keys and
     * any preceding and following menu items if specified for any of the added
     * menu items.
     *
     ...
     * @param defaultActionKey action key for menu items that don't have an
     * action explicitly assigned to them
     */
    ...
    private void appendMenuItemWithKey(final JMenu menu, final String menuItemKey, final String defaultActionKey) {
        if (getState(menuItemKey).isIncluded()) {
            ...
            if (precedingMenuItemKeys != null) {
                appendMenuItemsWithKeys(menu, precedingMenuItemKeys, defaultActionKey);
            }
            ...
            if (followingMenuItemKeys != null) {
                appendMenuItemsWithKeys(menu, followingMenuItemKeys, defaultActionKey);
            }
        }
    }
    ...
    /**
     * Add to the toolbar a group of toggle buttons which are mutually exclusive
     * with respect to selection state.
     *
     * @param toolbar The toolbar to which to add the toggle buttons.
     * @param groupKey The key for the group of buttons to add with a common
     * button group.
     */
    ...
    /**
     * Get the label for the specified item by its ID. Look for a control entry
     * with the item ID followed by ".label". If the label identifier cannot be
     * found then default to the item ID itself as the label.
     *
    ...
    /**
     * Add to the menu a group of radio button menu items which are mutually
     * exclusive with respect to selection state.
     *
    ...
    /**
     * Register all application commands (default and custom). Some of these
     * commands may be associated with documents.
     */
    ...
    /**
     * Register all document commands (default and custom). Some of these
     * commands may be associated with documents.
     *
     * @param document The document for which some commands may need to be
     * associated
     */
    ...
    /**
     * Subclasses may override this method to provide custom application
     * commands. Alternatively custom commands may be specified in subclasses of
     * XalDocument, XalWindow and ApplicationAdaptor for convenience.
     *
     ...
     * Alternatively custom commands may be specified in subclasses of
     * XalDocument, XalWindow and ApplicationAdaptor for convenience.
     *
     * @param document The document for which some commands may need to be
     * associated
     ...
     * Subclasses may override this method to provide custom document commands.
     * Alternatively custom commands may be specified in subclasses of
     * XalInternalDocument, XalInternalWindow and ApplicationAdaptor for
     * convenience.
     *
     * @param document The document for which some commands may need to be
     * associated
    ...
    /**
     * Register the action to be used by the commander. Every action should have
     * a unique name because the commander fetches actions by name.
     *
    ...
    /**
     * Register the action and button model to be used by the commander. Every
     * action should have a unique name because the commander fetches actions by
     * name.
     *
     * @param action The action to register.
    ...
    /**
     * Register the menu handler to be used by the commander. Associate the
     * specified name with the menu handler.
     *
