...
abstract class XalAbstractDocument implements Pageable {
     ...
     * suffixes identifying the files that can be written by the document. By
     * default this method returns the same types as specified by the
     * application adaptor.
     *
    ...
    /**
     * Override this method to register custom document commands if any. You do
     * so by registering actions with the commander. Those action instances
     * should have a reference to this document so the action is executed on the
     * document when the action is activated. The default implementation of this
     * method does nothing.
     *
    ...
    /**
     * Check if the document is empty. An empty document has no source file and
     * has not been edited.
     *
    ...
    /**
     * Generate and set the title for this document. By default the title is set
     * to the file path of the document or the default empty document file path
     * if the document does not have a file store.
     */
    ...
    /**
     * By default the file path to display for this document is set to the file
     * path of the document or the default empty document file path if the
     * document does not yet have a file store.
     *
     * @return the file path of the document or the default empty document file
     * path as appropriate
     */
     ...
     *
     * @return the new file name for new documents and the source's file path
    ...
    /**
     * Get the prefix for a new file (precedes timestamp) defaulting to the
     * application name. Subclasses can override this method to provide an
     * alternative prefix.
     *
     ...
     * Subclasses can override this method to provide an alternative name.
     *
     ...
     * @return "{baseName}." + the first writable document type or simply
     ...
     * Get the default file path to use for empty documents
     *
     * @return "Untitled." + the first writable document type or simply
     * "Untitled" if there are none
     */
    ...
    /**
     * Convenience method typically called in saveDocumentAs() to save a
     * document rooted in a data listener source.
     *
    ...
    /**
     * Subclasses may override this empty method to implement custom code called
     * by writeDataTo() after data has been successfully written to the
     * specified URL.
     *
     * @param dataRoot DataListener root of the document to save
     * @param url The URL to which the document should be saved.
     ...
     * unsaved changes. The default behavior is to warn the user if they have
     * unsaved changes. Override this method if you don't want the user to be
     * warned of unsaved changes.
     *
    ...
    /**
     * Dispose of custom document resources. Subclasses should override this
    ...
    /**
     * Called when the document will be closed. The default implementation does
     * nothing. Subclasses should override this method if they need to handle
     * this event.
     */
    ...
    /**
     * Hook indicating that the window will be opened. Called after the window
     * is created but before it is displayed by the document.
     */
    ...
    /**
     * If the main window is not already created, make it and perform any
     * initilization to bind the window to the document.
     */
    void initMainWindow() {
    ...
    /**
     * Implement the Pageable interface. Subclasses may override this
     * implementation to support custom document printing. Returns the number of
     * pages to print. This default implementation simply returns one for
    ...
    /**
     * Implement the Pageable interface. Subclasses may override this
     * implementation to support custom document printing. This default
     * implementation gets the page format from the default print manager.
     *
     * @param pageIndex The page number corresponding to the page format to
     * return
     * @return PageFormat for printing the specified page
     * @see xal.extension.application.PrintManager#getPageFormat
     * @throws java.lang.IndexOutOfBoundsException when the page index is out of
     * range
     */
    @Override
     ...
     * Implement the Pageable interface. Subclasses may override this
     * implementation to support custom document printing. This default
     * implementation simply returns a printable object that prints one page of
     * the document's main window.
     *
