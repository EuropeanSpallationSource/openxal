...
public class SimulationRunner {
    ...
    private void initiateOpenXAL(){
        //accelerator = XMLDataManager.acceleratorWithPath("/Users/nataliamilas/projects/sofia_main/main.xal");
    ...
    private void retrieveTrajectory(){
        ...
        for(int i=0; i<trajectory.numStates(); i++){
            ...
            if ((new Double(posX.get(i)) == 0.0) && (new Double(posY.get(i)) == 0.0)){
                posPhi.add(0.0);}
            else if (new Double(posX.get(i)) == 0.0){
                double phi = Math.asin(posY.get(i)/posR.get(i));
                posPhi.add(phi/Math.PI);}
            else if (new Double(posX.get(i)) > 0.0){
                double phi = Math.atan(posY.get(i)/posX.get(i));
                posPhi.add(phi/Math.PI);}
            else if (new Double(posY.get(i)) >= 0){
