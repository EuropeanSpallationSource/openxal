...
public class TestTrajectoryPersistence {
     ...
     */
    /**
     * Pull in the probe state data from the model.params file. We are going to
     * use just the initial state as the first and only state in the trajectory.
     *
    ...
    /**
     * Forces the probe to create a trajectory object (by calling
     * <code>Probe#initialize()</code>) then adds the initial state of the probe
     * to that trajectory object and returns it.
     *
     * @param probe probe object used to create the trajectory with one state
     *
     * @return trajectory for the given probe consisting of a single state (the
     ...
     */
    /**
     * Save the contained states of each trajectory container to a respective
     * file. This Test must pass in order for any other test to be successful.
     *
     * @since Dec 29, 2015, Christopher K. Allen
     */
    @Test
    ...
    /**
     * Test method for
     * {@link xal.model.probe.traj.Trajectory#loadFrom(xal.tools.data.DataAdaptor)}.
     */
    @Test
     ...
     */
    /**
     * Saves the given trajectory data to XML data adaptor with the given file
     * name (in the test directory for this class). A JUnit failure assertion is
     * thrown if any error occurs during the write.
     *
     * @param strFileName name of file to be written
     * @param traj trajectory object that provides data
     *
     * @return              <code>true</code> if successful, <code>false</code> if an
     * exception occurred
     *
     * @since Dec 29, 2015, Christopher K. Allen
     */
    ...
    /**
     * Creates a new trajectory object by reading in the XML data in the given
     * filename. The data is loaded using an XML data adaptor object. A JUnit
     * failure is asserted if an error occurs during the read or construction of
     * the new trajectory object.
     *
     * @param strFileName name of the file containing the trajectory formatted
     * data
     *
     * @return a new trajectory object of the correct type initialized with the
     * data in the given file, or <code>null</code> if a failure occurred.
     *
