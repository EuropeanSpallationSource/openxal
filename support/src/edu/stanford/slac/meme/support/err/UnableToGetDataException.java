...
/**
 * UnableToGetDataException indicates that an MEME service was not
 * able successfully satisfy a request for data.
 *
 * Specifically, use of this Exception helps an MEME service satisfy
 * the MEME rule that an unsuccessful request for data MUST finally
 * result in the request method throwing an RPCRequestExcption whose
 * text indicates that that specific service request method invocation
 * was "Unable to get data". An alternative to use of this general
 * UnableToGetDataException would be to use its parent
 * AidaRPCRequestException with an unable to get data message specific
 * to the kind of data being acquired, like "Unable to get archive
 * data".
 *
...
public class UnableToGetDataException extends RPCRequestException
{
	 ...
	 * Suitable for request method
	 */
	public UnableToGetDataException(Status status, 
					String message, Throwable cause) 
	{
		super(status.getType(), 
		      message + " - " +
		      status.getMessage() + "; "+
		      (cause.getMessage() != null ? 
		    		  cause.getMessage() :
