...
public class RdbClient 
{
	...
	private final static Structure uriStructure =
		fieldCreate.createStructure(MemeNormativeTypes.NTURI_ID,
	...
	{	
			...
			if (pvResult != null)
			{
				...
				if (!type.equals(MemeNormativeTypes.NTTABLE_ID))	
				{
					logger.log(Level.SEVERE, 
					    "Unable to get data: unexpected data structure returned from "+
				...
				for (PVField pvColumnIterator : pvColumns)
				{
					...
					if (pvColumnIterator.getField().getType() == Type.scalarArray)
					{
						...
						} else
						{
							...
							logger.log( Level.SEVERE, "Unexpected array type returned from "+
							    CHANNEL_NAME+ 
		...
		catch (RPCRequestException rre)	
		{
			logger.log(Level.SEVERE, "Acquisition of "+args[0]+" query was not successful.\n"+
				   "Channel " +CHANNEL_NAME+ " connected but issued exception.: \n"+
				   rre.getMessage());
		}
		catch (IllegalStateException rre)	
		{
			logger.log(Level.SEVERE, "Acquisition not successful, failed to connect to "+CHANNEL_NAME+": \n"+ 
				   rre.getMessage());
		}
		finally
		{
	...
	private static void parseArguments( String[] args )
	{
		...
		if ( args[_STYLE_ARGN] != null && args[_LABELS_ARGN] != null )
		{
			if ( _LABELS_WANTED.equals(args[_LABELS_ARGN]) )
				...
				System.err.println( "Unexpected value of labels argument; it must be given as " +
						    _LABELS_WANTED + 
