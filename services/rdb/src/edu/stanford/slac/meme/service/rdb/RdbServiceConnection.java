...
public class RdbServiceConnection
{
		...
		Logger.getLogger(RdbServiceConnection.class.getName());
	...
	private static final int MAX_RETRIES = 2; // Try a SQL query at most 2 times
        ...
        private static final String SQLSELECT = 
	       "SELECT TRANSFORM FROM AIDA_NAMES WHERE UPPER(INSTANCE) = '%s' " +
	...
	private static final String NORESULTSETMETADATA = 
        ...
        private static final String INVALIDRDBQUERYNAME = 
		"Invalid syntax of RDB query name (%s), at least one colon expected";
	private static final String UNABLETOTRANSFORM = 
		"Failed to find a SQL query name matching the given name";
	private static final String TOOMANYROWMATCHES =
		"DATABASE DATA ERROR EDETCTED: More than row matches query name.";
	private static final String ZEROROWMATCHES =
		"No query name found in database matching given name.";
	private static final String RETRYMSG = 
		"Failed to execute SQL query, retrying with new java.sql.Connection.";
	 ...
	 * Pattern for recognizing "&instance" in a query translation. This syntax is
         ...
         * transforms to the MEME db, although the substitution is of course done
         * entirely in java.
         * NOTE: the name "instance" for the substitution is really wrong, it should
         * be called "entity". 
	 */
	 ...
	 * server startup, and the important part (getConnection) can be 
	 * redone at any time if the connection
	...
	private void init( String service_name )
	{
		...
		catch (Throwable ex)
		{
			RuntimeException runtimeException = 
			    new RuntimeException("Failed to initialize service: "+ 
						 service_name, ex);
	 ...
	 * 
	 * If the connection is non-null, then the old connection is first
	 * closed. This part is include so that this routine can be used to renew a
	 * stale connection.
	 */
	private synchronized void getConnection()
	{
		...
		{
			//
			String url = System.getProperty("CONNECTION_URI_PROPERTY",
		...
		catch (Throwable ex)
		{
			logger.log(Level.SEVERE, 
				   "Failed to initialize connection to database.", ex);
		}
	...
	private synchronized void closeConnection()
	{
		...
		{
			logger.log(Level.SEVERE, 
				   "Failed to close connection to database.", ex);
		}
	}
	 ...
	 * query name, as it is given in the data source to which the server connects
	 * (probably a releational database).
	 * 
	 * @param queryName identifier of the SQL query; what the end user entered
	 * @return the SQL select statement that corresponds to the queryName 
	 *         input parameter  
	 */
	public String instanceToQuery(String queryName) throws UnableToGetDataException
	{
		...
		if (firstSlash == -1)
			throw new UnableToGetDataException(
				String.format(INVALIDRDBQUERYNAME,queryName));
		String entity=queryName.substring(0,firstSlash);
		String attribute=queryName.substring(firstSlash+1);
		String queryQuery = 
			String.format(SQLSELECT, entity.toUpperCase(), attribute.toUpperCase());
		...
		try
		{
			...
			if ( rowsM > 1 ) 
			{
				logger.severe(TOOMANYROWMATCHES +
					      " Aborting."+
					      " Lookup query used: "+ queryQuery);
				throw new UnableToGetDataException( TOOMANYROWMATCHES );
			}
			if ( rowsM < 1 )
			{
				logger.info(ZEROROWMATCHES +
					      " Lookup query used: "+ queryQuery);
			...
			if ( sqlqueryResultSet.getString(QRYCOLUMNNUM).charAt(0) == '/' )
				sqlQuery=
				      substitute(sqlqueryResultSet.getString(
					 QRYCOLUMNNUM).substring(1), entity, attribute );
			else
				sqlQuery=sqlqueryResultSet.getString(QRYCOLUMNNUM);
		...
		finally
		{
			...
			{
				logger.log(Level.SEVERE, 
			"Failed to free JDBC resources for query '" + queryQuery + "'.", e);
			}
		}
	}
	...
	public PVStructure getData(String query) throws UnableToGetDataException
	{
		...
		try
		{
			...
			for (int colj = 1; colj <= columnsN; colj++)
			{
				...
				default:
				{
					...
					//		ScalarType.pvString);
					// myArr.add(colField);
					PVStringArray valuesArray = 
						(PVStringArray)pvFields[colj-1]; 
		...
		{
			throw new UnableToGetDataException(
				"Failed to process SQL query: " + query, e);
		} 
		finally
		{
			// Free JDBC resources. 
			...
			{
				logger.log(Level.SEVERE, 
					   "Failed to free JDBC resources for query: " +
					   query, e);
			}
		}
		...
		do
		{
			...
			catch (Throwable ex)
			{
				...
				else
				{
					bRetry = false;
					String suppl = 
						"Failed to execute SQL query " + sqlString;
	 ...
	 * The most common mapping, and all that supported for now, is replacing
         ...
         * string should be made by this method. 
	 *
	 * @param original The sql query to be executed on the database before
         *        substitutions.
	 * @param entity The query "instance" part (eg device-name, eg
	 *                 "XCOR:LI21:401")
