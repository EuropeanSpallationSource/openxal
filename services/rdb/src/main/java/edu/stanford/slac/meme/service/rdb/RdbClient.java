...
/**
 * RdbClient is a simple client of the rdbService; the rdbService is intended to help get data out of relational
 * databases like Oracle, mySQL, via EPICS V4.
 * 
...
public class RdbClient {
    ...
    private static final String CHANNEL_NAME = "rdb"; // Name of service to contact.
    ...
    private final static Structure uriStructure = fieldCreate.createStructure(MemeNormativeTypes.NTURI_ID,
            new String[] { "scheme", "query" }, new Field[] { fieldCreate.createScalar(ScalarType.pvString),
                    queryStructure });

    /**
     * Main of rdbService client takes 1 argument, being the name of a SQL query that the server side will understand.
     * 
     * @param args
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            if (pvResult != null) {
                ...
                /*
                 * OK, so we know we got back an NTTable. Now we have to decode it and display it as a table. The datum
                 * pvResult that came back from the rdbService should be of EPICS V4 normative type NTTable. That is,
                 * pvResult is a PVStructure whose specific shape conforms to the definition NTTable
                 * (http://epics-pvdata.sourceforge.net/normative_types_specification .html). NTTable is composed of two
                 * fields; a string array field of column titles, and a structure containing N array fields each being
                 * one column of data. Further, we know that rdbService only returns columns of type PVDoubleArray,
                 * PVLongArray, PVByteArray, or PVStringArray. We use the introspection interface of PVStructure to
                 * determine which.
                 */
                ...
                for (PVField pvColumnIterator : pvColumns) {
                    ...
                    if (pvColumnIterator.getField().getType() == Type.scalarArray) {
                    	...
                    	default: 
                            ...
                            // that all possible terminals of a non-terminal N-type field
                            // are used by every client, only that every client using a
                            // N-tpype SHOULD handle every possible implied design of that
                            // N-type. Furthermore, handling specifically includes trapping
                            // the condition that the implied design is not semantically
