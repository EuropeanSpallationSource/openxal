...
public class TripMonitorService implements TripMonitorPortal {
    ...
    /**
     * Get the list of all PVs we are attempting to monitor and log. The
     * information is returned as a list of channel info tables (one entry for
     * each PV). The channel info table has the CHANNEL_PV and
     * CHANNEL_CONNECTION keys and provides the signal name and the connection
     * status of a channel.
     *
     * @param monitorName name of the trip monitor for which to get the channel
     * information
     * @return list of all PV info tables for the PVs we are attempting to
     * monitor and log
     */
    ...
    /**
     * Get the number of trip records in the history buffer of the specified
     * trip monitor
     *
