...
public class ChannelMonitor {
    ...
    /**
     * Register the listener as a receiver of channel events from the wrapped
     * channel
     *
    ...
    /**
     * Unregister the listener as a receiver of channel events from the wrapped
     * channel
     *
     ...
     *
     * @param record the new record from which to get the latest value to test
     * for a trip
    ...
    protected void makeMonitor() {
        try {
            monitor = CHANNEL.addMonitorValTime(new IEventSinkValTime() {
                /**
                 * Process the monitor event by caching the latest channel
                 * record
                 *
     ...
     * and disposing of the messaging resources.
     */
    public void dispose() {
        synchronized (EVENT_LOCK) {
            if (connectionListener != null) {
