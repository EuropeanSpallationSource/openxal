...
/**
 * The optics Service defines the server side of an MEME EPICS V4 service
 * for accessing a relational database which contains the twiss parameters
 * and orbit response matrices (R-matrices) of a particle accelerator
 * mathematical model.
 *
 * In MEME, optics database "queries" are valued as the name of a accelerator
 * device followed by the optics data you want to get for that device.  
 * <pre>
 * QUAD:LI23:13/R - returns NTMatrix 
 * BPMS:UND1:32/twiss - returns NTTable </pre>
 * 
 * @author Greg White, 25-Oct-2013 (greg@slac.stanford.edu)
 * @see [1] <a href="http://epics-pvdata.sourceforge.net/alpha/normativeTypes/normativeTypes.html#ntmatrix">
...
public class OpticsService
{
	...
	private static final Logger logger = 
		Logger.getLogger(OpticsService.class.getPackage().getName());
	...
	private final static Structure rmatStructure =
		fieldCreate.createStructure(MemeNormativeTypes.NTMATRIX_ID,
		new String[] { "value", "dim" },
		new Field[] { 
	...
	private static double[] fakeTwissData = { 
		0.22, 
		20.8572411437, 29.3321341325, 12.5804392017, 
		0.0436279229998, -0.0193296502818,
		15.1387492999, 31.0719654674, -31.6073259725, 0.0, 0.0, 
		2059.071169, 0.108, 0.054, 526};
	private static double[] fakeRmatData = {
		2.09624160473, 0.804154556226, 0.0, 0.0, -1.06385858999, -0.0112465957359,
		-0.966309173538, -0.357701041448, 0.0, 0.0, 0.474782969028, 0.00498288131746,
                0.0, 0.0, -0.303419179617, 1.28893518781, 0.0, 0.0, 
		0.0, 0.0, -0.314544894536, 1.24631295859, 0.0, 0.0, 
		-0.00130328580005, 2.17740006796E-4, 0.0, 0.0, 0.14234033399, 0.00291949986906,
		-0.0295967119844, -0.0246787398374, 0.0, 0.0, -21.9009846796, -0.257784349164};

	/**
	 * The implementation class of the OpticsService RPCService, which
	 * gets transverse beam optics data, specifically Twiss params and
	 * orbit response matrices, for a given device, per user requests.
	 *  
	...
	private static class OpticsServiceImpl implements RPCService
	{
		...
		/**
		 * Construct and return the requested database data, given
				...
				throws RPCRequestException
		{
			...
			try 
			{
				...
				//
				PVStructure pvQuery = 
				    pvUri.getStructureField("query");
				logger.fine("pvQuery received= " + pvQuery);

				PVString pvQueryName =
					pvQuery.getStringField("q");
				if (pvQueryName == null)
					throw new RPCRequestException(
					  StatusType.ERROR,
				          String.format(
					   Message.MISSINGREQUIREDARGLVAL,"q"));
				String pvname = pvQueryName.get();
				if (pvname == null)
					throw new RPCRequestException(
                                          StatusType.ERROR,
			      	          String.format(
					   Message.MISSINGREQUIREDARGRVAL,"q"));
				...
				else if ( entity.attribute().equals("R") )
				{
					...
					// interface of rmat defined above.
				        result = 
					  PVDataFactory.getPVDataCreate().
					    createPVStructure(rmatStructure);
					PVDoubleArray rmatpv = (PVDoubleArray) 
					  result.getScalarArrayField("value",
					    ScalarType.pvDouble);
					PVIntArray dimpv = (PVIntArray) 
					  result.getScalarArrayField("dim",
					    ScalarType.pvInt);	
				...
				else 
				{
					...
					throw new IllegalArgumentException( 
						OpticsMessage.INVALIDATTRIBUTE + 
						" : "+entity.attribute()); 
				}
			 
				
				logger.fine(
			  "Successfully retrieved model data interface of PVStruture");
		...
		/**
		 * Examines the "parameters", passed from the client as part of the MEME
		 * query.
		 ...
		 * found to be invalid.
		 */
		private String[] parseArgs( PVStructure pvQuery )
			throws IllegalArgumentException
		{
			// Parameters for query, initialized to defaults.
			String[] paramNames=
				{"b","type","pos","posb","run","fakedata"};
			String[] paramValues=
				{"NULL","EXTANT","END","BEG","GOLD","NULL"};

			// TODO: Add a check that an unreognized parameter (-a) is given.
		       
			/* Extract the parameters from the NTURI query PVStructure
			   given to service, and override defaults.
			*/
			for ( int i=0; i<paramNames.length; i++ )
			{
				...
				{
					PVString pvParam = 
					  pvQuery.getStringField(paramNames[i]);
					if ( pvParam != null )
						paramValues[i] =
						  pvParam.get().trim().toUpperCase();
				}
			}

			/* Fix cases where 2 params have been given in contradiction.
			 * If an explicit run number is given, then warn that the 
			 ** type of it is irrelevant.
			 */
			if (pvQuery.getSubField(paramNames[TYPE_PARAM]) != null &&
			    pvQuery.getSubField(paramNames[RUN_PARAM]) != null &&
			    Pattern.matches("^(0+)?[1-9][0-9]+$",
			...
			{
				throw new IllegalArgumentException(
					OpticsMessage.TYPE_PARAM_VAL_ILLEGAL);
			}
			...
			{
				throw new IllegalArgumentException(
					OpticsMessage.POS_PARAM_VAL_ILLEGAL);
			}
			...
			if ( paramValues[RUN_PARAM].equalsIgnoreCase( "NULL" ) || 
			     paramValues[RUN_PARAM].equalsIgnoreCase( "0" ) ||
			     paramValues[RUN_PARAM].equalsIgnoreCase( "LATEST" ) )
			{
				paramValues[RUN_PARAM] = "LATEST";
			}
			else if ( paramValues[RUN_PARAM].equalsIgnoreCase( "GOLD" ) ||
				  paramValues[RUN_PARAM].equals("1") )
			{
				...
				catch ( Exception n )
				{
					IllegalArgumentException ex = 
						new IllegalArgumentException(
							OpticsMessage.ILLEGALRUN_PARAM);
					throw ex;
				}
			}
			...
			{
				logger.fine(
                                  "Query made with following parsed parameters:");
				for ( int i = 0; i<paramValues.length; i++)
					logger.fine("\t" + paramNames[i] + " = " + 
						    paramValues[i] );
			}
		 ...
		 */
		private double[] get_twissA( Entity a, String[] params )
			throws UnableToGetDataException
		{
			...
			String  typeParam =  params[TYPE_PARAM],
				...
				fakeDataParam = params[FAKEDATA_PARAM];
			...
			else 
			{
				...
				if ( runID == 0 )
				{
					throw new UnableToGetDataException( 
						a.instance() + 
						" optics not found in "+runIdParam);
				}
		 ...
		 * separate for the sake of explicit coding.
		 */
		private double[] get_rmatA( Entity a, String[] params )
			throws UnableToGetDataException
		{
		...
		// get_twiss ESS
		private double[] get_twiss( Entity q, 
					    String posParam, 
					    Integer runId 
			                   )
			throws UnableToGetDataException
		{
			...
			final String GETTWISS=
			  ...
			  "from \"MACHINE_MODEL\".\"ELEMENT_MODELS\" e, "+
				"\"MACHINE_MODEL\".\"RUNS\" r "+ 
			  "where "+
				"e.\"RUNS_ID\"=%d and e.\"ELEMENT_NAME\"='%s' "+
				"and e.\"RUNS_ID\" = r.\"ID\" "+
		...
		// get_rmat ESS
		private double[] get_rmat( Entity q, 
					   String posParam, 
					   Integer runId )
			throws UnableToGetDataException
		{
		 ...
		 */
		private double[] get_rmat_AtoB( Entity q, String[] params )
			throws UnableToGetDataException
		{
			...
			else 
			{
				...
				if ( runIdD == 0 )
				{
					throw new UnableToGetDataException(
						d.instance() + 
						" was not found in a model of type " + 
						typeParam );  
				}
			...
			if (logger.isLoggable(Level.FINE)) printMatrix( "rmatA", A );
			Matrix B = new Matrix( rmatB, 6 ).transpose();
			if (logger.isLoggable(Level.FINE)) printMatrix( "rmatB", B );
			Matrix RmatAB = B.times(A.inverse());
			if (logger.isLoggable(Level.FINE)) printMatrix( "RmatAB", RmatAB); 
		 ...
		 *
		 * @param instance An EPICS channel access addressable device name, or
		 * more specifically an XAL element EPICS name, which should match an
				       ...
				       String run)
		{
			...
			{
				logger.warning("Problem running database query for getting run id: " +
					       ex.toString());
				return 0;
			}
		 ...
		 * across X and Y.
		 */
		private boolean isDownstream( Entity a, Entity b, String[] params )
			throws UnableToGetDataException
		{
			...
			if ( A_twiss[ PSIX ] > B_twiss[ PSIX ] && 
			     ...
			     A_twiss[ PSIY ] > B_twiss[ PSIY ] )
				throw new
				    UnableToGetDataException(OpticsMessage.INCONSISTENT_PSI);
	...
	public static void main(String[] args) throws PVAException
	{
		...
		{
			String _msg = "MEME_MODE property value given \""+prop+
			    "\" is not valid;"+
			    " valid MEME_MODES are: "+Mode.modeMap.keySet();
			logger.severe( _msg ); 
		...
		prop = System.getProperty( "DATA_MODE",
				"REAL" ).toUpperCase();;
		...
		{
			String _msg = "DATA_MODE property value given \""+prop+
			    "\" is not valid;"+
		        	" valid DATA_MODES are: "+DataMode.modeMap.keySet();
			logger.severe( _msg ); 
			logger.severe("Aborting, previous errors prohibit execution");
			throw new PVAException( _msg );
		}
		...
		server.registerService(OPTICS_SERVICE_NAME, 
				       new OpticsServiceImpl(opticsConnection));
		logger.info("SERVICE \""+OPTICS_SERVICE_NAME +
			    "\" is operational in "+
			    Mode.modes[meme_mode]+" mode, returning "+
			    DataMode.datamode_names[data_mode]+" data.");		
