 ...
 * Usage:
 *
 * java TestServerClient quad45:bdes/history
...
public class TestServerClient 
{
	...
	{	
			...
			if (pvResult != null)
			{
				...
				if (!type.equals(NTTABLE_ID))	
				{
					logger.log(Level.SEVERE, 
						   "Unable to get data: unexpected data "+
				...
				for (PVField pvColumnIterator : pvColumns)
				{
					...
					if (pvColumnIterator.getField().getType() ==
					    Type.scalarArray)
					{
					    ScalarArray scalarArray = 
						(ScalarArray) pvColumnIterator.getField();
					    // Double array
					    if (scalarArray.getElementType() ==
						ScalarType.pvDouble)
					    {
						PVDoubleArray pvDoubleArray =
						    (PVDoubleArray) pvColumnIterator;
						namedValues.add( 
						    fieldName,
						    GetHelper.getDoubleVector( 
						       pvDoubleArray ));
					    } else if (scalarArray.getElementType() ==
						       ScalarType.pvLong)
					    {
						PVLongArray pvLongArray = 
						    (PVLongArray) pvColumnIterator;
						namedValues.add(
						    fieldName,
						...
						PVStringArray pvStringArray = 
						    (PVStringArray) pvColumnIterator;
						namedValues.add(
						    fieldName,
						...
						//
						logger.log( Level.SEVERE, 
							    "Unexpected array type returned from "+
							    CHANNEL_NAME
					...
					{
					    logger.log( Level.SEVERE, 
							"Unexpected non-array field returned from "+
							CHANNEL_NAME
							+ ".\n Field named \'"+ fieldName + 
							"\' is not of scalarArray type, "
				...
				// it to System.out.
				//
				NamedValuesFormatter formatter = NamedValuesFormatter.create(
                                  NamedValuesFormatter.STYLE_COLUMNS );
		...
		{
			logger.log(Level.SEVERE, 
				   "Acquisition of "+args[0]+" query was not successful.\n"+
				   "Channel " +CHANNEL_NAME+ 
			       " connected but issued exception. Check validity and spelling:", rre);
		}
		catch (IllegalStateException rre)	
		{
			logger.log(Level.SEVERE, 
				   "Acquisition not successful, failed to connect to "+
				   CHANNEL_NAME, rre);
		}
		finally
		{
