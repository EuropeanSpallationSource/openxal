...
/**
 * TestService is an EPICS V4 RPC service used exclusively for testing the functionality of an MEME network. All the
 * data this service returns originates right in this code, so if a client can get data from this service, the network
 * works, and if it can't the the network itself doesn't work.
 * <p>
 * TestService is a copy of the class ServiceAPIServer, distributed with EPICS V4 in exampleJava/illustrations/. As
 * ServiceAPIServer, it pretends to be an Archive service, that knows only 1 PV (quad45:bdes) and over a very short time
 * range (2011-09-16T00.03.42 - 2011-09-16T19.45.50). So, the only eget queries like the following will be valid:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * ** It's the memetestservice to distinguish it from the shipped pvAccess test server which also calls itself
 * "testserver" on the network.
 *
...
public class TestServer {
    ...
    private final static Structure resultStructure = fieldCreate.createStructure(MemeNormativeTypes.NTTABLE_ID,
            new String[] { "labels", "value" }, new Field[] { fieldCreate.createScalarArray(ScalarType.pvString),
                    valueStructure });
    ...
    public static void main(String[] args) throws PVAException {
        ...
        String server_name = System.getProperty("SERVER_NAME", SERVER_NAME);
        ...
        server.registerService("qm14:twiss", new TwissServiceImpl());
        logger.info("SERVICE \"" + "qm14:twiss" + "\" is operational.");

        server.registerService("bpm21301:twiss", new TwissServiceImpl());
    ...
    static class TwissServiceImpl implements RPCService {
        ...
        public PVStructure request(PVStructure pvUri) throws RPCRequestException {
            ...
            PVStructure twissParams = PVDataFactory.getPVDataCreate().createPVStructure(twissStructure);
            ...
            try {
                ...
                if (elementname.compareToIgnoreCase("qm14:twiss") == 0) {
                    ...
                    throw new MEMERequestException(String.format(
                            "Service registered for '%s' does not know how to process that name!", elementname));

            } catch (Exception ex) {
                iss = new UnableToGetDataException("Unable to get twiss data for " + elementname, ex);
    ...
    static class TestServiceImpl implements RPCService {
        ...
        public PVStructure request(PVStructure uri) throws RPCRequestException {
            ...
            try {
                ...
                PVStringArray datetimesArray = (PVStringArray) archiveDataTbl.getScalarArrayField("times",
                        ScalarType.pvString);
                PVDoubleArray readingsArray = (PVDoubleArray) archiveDataTbl.getScalarArrayField("readings",
                        ScalarType.pvDouble);
                logger.fine("Successfully retrieved data interface of PVStruture");
                ...
                if (pvname.equalsIgnoreCase(archivedPVname)) {
                    ...
                    if (j == 0) {
                        String msg = "No matching data beteen %s and %s";
                        throw new MEMERequestException(String.format(msg, calStarttime, calEndtime));
                    }
                ...
                iss = new UnableToGetDataException("Unable to get test archive data", ex);
                logger.severe(iss.getMessage());
                throw iss;
            }
            return result;
        ...
        private String getQueryArg(PVStructure pvUriQuery, String argname) throws RPCRequestException {
            ...
            PVString pvQueryArg = pvUriQuery.getStringField(argname);

            if (pvQueryArg == null) {
                // throw but don't log - per MEME logging rules.
                iss = new RPCRequestException(StatusType.ERROR,
                        String.format("Missing required argument '%s'", argname));
                throw iss;
            } else {
                argValue = pvQueryArg.get();
                if (argValue.length() == 0)
                    throw new MEMERequestException(String.format("Missing required value for argument '%s'", argname));
            }
            // Tracing MUST be logged as FINE.
            logger.fine("Argument \'" + argname + "\' received" + ", value = \'" + argValue + "\'");
     ...
     * 
     * This is fake date/time stamps and associated fake data from which client queries can be drawn.
     */
    private static void initFakeArchiveData() {
        ...
        try {
            ...
            System.err.println("Error Constructing fake data at initialization;" + ex.toString());
            System.exit(1);
        }
    }
}
