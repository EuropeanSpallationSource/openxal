...
public class MPSService implements MPSPortal {
    ...
    /**
     * Get the process ID of the process in which the service runs or 0 if it is
     * unknown. When the java process is launched, the calling script needs to
     * pass the process ID as a java property (exec java -Dpid=$$ -jar
     * mpsservice.jar) for this to work.
     */
    ...
    /**
     * Get the list of MPS latch types. Monitors are listed as an array and
     * their index in this array can be used in several methods to reference a
     * specific monitor.
     *
     ...
     *
     * @param monitorIndex index of the monitor that should restart its
     * correlator
     */
    ...
    /**
     * Get an arry of timestamps corresponding to the different event ids. The
     * values are Date objects that indicate the time the last event of the
     * specified type had happened. This method may be used by clients to
     * determine if their information is current.
     *
     ...
     * information is returned as a list of channel info tables (one entry for
     * each PV). The channel info table has the CHANNEL_PV_KEY and
     * CHANNEL_CONNECTED_KEY keys and provides the signal name and the
     * connection status of a channel.
     *
     * @param monitorIndex index of the monitor whose channel info is requested
     * @return The list of all PV info tables for the MPS PVs we are attempting
     * to monitor and log
     */
     ...
     *
     * @param monitorIndex index of the monitor whose statistics are to be
     * fetched
    ...
    /**
     * Get the list of MPS events since the specified time. Unfortunately,
     * XML-RPC does not preserve the millisecond accuracy of time, so we must
     * pass a string representation.
     *
     * @param monitorIndex index of the monitor whose statistics are to be
     * fetched
