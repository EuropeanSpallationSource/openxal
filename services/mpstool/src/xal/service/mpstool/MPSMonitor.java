...
public class MPSMonitor {
    ...
    /**
     * Filter used to set the amount of missing MPS PVs allowed to constitute a
     * legitimate correlation set
     */
    ...
    /**
     * Map of first hit trip statistics keyed by PV. This map gets cleared
     * daily.
     */
    ...
    /**
     * Reload the signals to monitor from the data source. Stop the correlator,
     * recreate a new correlator with the new signals and restart the
     * correlator.
     */
     ...
     *
     * @param mpsSignal The MPS signal for which to get the corresponding input
     * monitor
    ...
    /**
     * Stops the poster from posting, but the correlator is still running behind
     * the scene
     */
    ...
    /**
     * Get the timestamp of the latest MPS event. This can be used by clients to
     * determine if their first hit log is current.
     *
    ...
    /**
     * Get the timestamp of the last MPS channel event. This can be used by
     * clients to determine if their list of channels is up to date. This
     * timestamp changes when the list of monitored channels changes or the
     * connection state of one or more monitored channels changes.
     *
    ...
    /**
     * Increment the number of times the MPS signal has been the first to trip
     * in correlated MPS trips. The stats are only valid for the present day.
     *
    ...
    /**
     * Increment the number of times the MPS signal tripped. The stats are only
     * valid for the present day.
     *
    ...
    /**
     * Increment the number of times the MPS signal's input has tripped. The
     * stats are only valid for the present day.
     *
    ...
    /**
     * Update the statistics that maintains the number of MPS trips per MPS
     * signal to include the specified MPS event.
     *
    ...
    /**
     * Get the latest values for the specified input monitors. Check which
     * inputs have tripped and increment their daily trip statistics
     * accordingly.
     *
    ...
    /**
     * Check to see if the day has changed since the the startTime. If so, reset
     * the startTime to be the start of the new day and clear the daily
     * statistics.
     */
    ...
    /**
     * Reset the daily statistics by clearing them and setting the startTime to
    ...
    /**
     * Get the list of MPS events which have occured since the specified time.
     * Events which occur before or at the specified time are excluded from the
     * list.
     *
     ...
     * @return the top count trip records ordered from most to least MPS first
     * hits
     */
    ...
    /**
     * Create and start a new timer for checking every hour whether to update
    ...
    private class MPSConnectionHandler implements ConnectionListener {
         ...
         * Constructor
         *
         * @param recordFilter The record filter to apply to connected channels
         * with the correlator
         */
        ...
        /**
         * Request that the channel monitor events be correlated within the
         ...
         * established. Add the channel to the correlator with the record
         * filter. Update the connection event timestamp.
         *
        ...
        /**
         * Indicates that a connection to the specified channel has been
         * dropped. Update the connection event timestamp.
         *
    ...
    private class InputConnectionHandler implements ConnectionListener {
        ...
        /**
         * Request connections for the channel wrappers and monitor their
         * connection events.
         *
